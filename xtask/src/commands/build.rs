...
pub struct DeepcodeBuildCmdArgs {
        ...
        Context::NoStd => {
            ...
            .try_for_each(|build_target| {
                ...
                if *build_target == ARM_NO_ATOMIC_PTR_TARGET {
                    // Temporarily remove `deepcode-autodiff` from building with the
                    // target `thumbv6m-none-eabi` as it requires enabling the
                    // `arbitrary_self_types` feature for the
                    // `clone_if_require_grad` method of
                    // `deepcode-autodiff::graph::Node`
                    crates.retain(|&v| v != "deepcode-autodiff");
        ...
        Context::Std => {
            if args.ci {
                ...
                if std::env::var("DISABLE_WGPU").is_ok() {
                    args.exclude.extend(vec!["deepcode-wgpu".to_string()]);
                };
            }
            // Build workspace
