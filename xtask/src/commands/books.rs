...
pub(crate) enum BookKind {
    ///  Deepcode Book, a.k.a. the guide, made for the Deepcode users.
    Deepcode(BookKindArgs),
...
impl Book {
    const BDEEPCODE_BOOK_NAME: &'static str = "Deepcode Book";
    const BDEEPCODE_BOOK_PATH: &'static str = "./deepcode-book";
    ...
    pub(crate) fn run(book_arg: &BookKind) -> anyhow::Result<()> {
        let (book, command) = match book_arg {
            BookKind::Deepcode(args) => (
                Self {
                    name: Self::BDEEPCODE_BOOK_NAME,
                    path: Path::new(Self::BDEEPCODE_BOOK_PATH),
                },
                &args.command,
            ),
