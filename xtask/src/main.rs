...
pub enum Command {
    /// Run commands to manage Deepcode Books.
    Books(commands::books::BooksArgs),
    /// Build Deepcode in different modes.
    Build(commands::build::DeepcodeBuildCmdArgs),
    /// Test Deepcode.
    Test(commands::test::DeepcodeTestCmdArgs),
}
