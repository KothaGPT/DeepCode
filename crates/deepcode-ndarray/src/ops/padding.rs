...
pub(crate) fn apply_padding_4d<E: Element>(
    ...
    x_new = NdArrayOps::slice_assign(
        x_new,
        &[
            deepcode_tensor::Slice::from(0..batch_size),
            deepcode_tensor::Slice::from(0..input_channels),
            deepcode_tensor::Slice::from(padding_height..height + padding_height),
            deepcode_tensor::Slice::from(padding_width..width + padding_width),
        ],
        x,
    );
...
pub(crate) fn apply_padding_5d<E: Element>(
    ...
    x_new = NdArrayOps::slice_assign(
        ...
        &[
            deepcode_tensor::Slice::from(0..batch_size),
            deepcode_tensor::Slice::from(0..input_channels),
            deepcode_tensor::Slice::from(padding_depth..depth + padding_depth),
            deepcode_tensor::Slice::from(padding_height..height + padding_height),
            deepcode_tensor::Slice::from(padding_width..width + padding_width),
        ],
        x,
