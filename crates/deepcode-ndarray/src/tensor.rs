...
macro_rules! execute_with_dtype {
    (($lhs:expr, $rhs:expr),$element:ident,  $op:expr, [$($dtype: ident => $ty: ty),*]) => {{
        let lhs_dtype = deepcode_tensor::TensorMetadata::dtype(&$lhs);
        let rhs_dtype = deepcode_tensor::TensorMetadata::dtype(&$rhs);
        match ($lhs, $rhs) {
            $(
...
impl QTensorPrimitive for NdArrayQTensor {
    ...
    fn default_scheme() -> QuantScheme {
        QuantScheme::default().with_store(deepcode_tensor::quantization::QuantStore::Native)
    }
}
...
mod tests {
    ...
    use super::*;
    use deepcode_common::rand::get_seeded_rng;
    use deepcode_tensor::{
        Distribution,
