...
use deepcode_collective::{CollectiveError, PeerId, ReduceOperation, all_reduce};
...
use deepcode::module::{AutodiffModule, ParamId};
...
impl GradientsParams {
    ...
    pub fn all_reduce<B: Backend>(
        ...
        for id in ids {
            ...
            let grad = match grad {
                deepcode::tensor::TensorPrimitive::Float(grad) => {
                    let grad = all_reduce::<B>(peer_id, grad, op)?;
                    deepcode::tensor::TensorPrimitive::Float(grad)
                }
                deepcode::tensor::TensorPrimitive::QFloat(_grad) => {
...
mod tests {
    ...
    use deepcode::module::{Module, list_param_ids};
    use deepcode::tensor::{Distribution, backend::Backend};
