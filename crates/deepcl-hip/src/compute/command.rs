...
impl<'a> Command<'a> {
    ...
    pub fn write_to_gpu(&mut self, descriptor: CopyDescriptor, data: &[u8]) -> Result<(), IoError> {
        ...
        if rank > 1 {
            ...
            unsafe {
                let status = deepcl_hip_sys::hipMemcpy2DAsync(
                    resource.ptr,
                    stride_bytes,
            ...
            unsafe {
                let status = deepcl_hip_sys::hipMemcpyHtoDAsync(
                    resource.ptr,
                    data as *const _ as *mut _,
                    data.len(),
                    current.sys,
...
pub(crate) unsafe fn write_to_cpu(
    ...
    if rank <= 1 {
        let status = unsafe {
            deepcl_hip_sys::hipMemcpyDtoHAsync(
    ...
    unsafe {
        ...
        if status != HIP_SUCCESS {
            let status = deepcl_hip_sys::hipMemcpyDtoHAsync(
                bytes.as_mut_ptr() as *mut _,
                resource_ptr,
                bytes.len(),
                stream,
