use deepcl_hip_sys::HIP_SUCCESS;
...
pub struct Fence {
    event: deepcl_hip_sys::hipEvent_t,
}
...
impl Fence {
    ...
    pub fn new(stream: deepcl_hip_sys::hipStream_t) -> Self {
        let mut event: deepcl_hip_sys::hipEvent_t = std::ptr::null_mut();
        unsafe {
            let status = deepcl_hip_sys::hipEventCreateWithFlags(
                &mut event,
                deepcl_hip_sys::hipEventDefault,
            );
            assert_eq!(status, HIP_SUCCESS, "Should create the stream event");
            let status = deepcl_hip_sys::hipEventRecord(event, stream);
    ...
    /// The [stream](hipStream_t) must be initialized.
    #[allow(unused)]
    pub fn wait_async(self, stream: deepcl_hip_sys::hipStream_t) {
        unsafe {
            let status = deepcl_hip_sys::hipStreamWaitEvent(stream, self.event, 0);
            assert_eq!(
                status, HIP_SUCCESS,
                "Should successfully wait for stream event"
            );
            let status = deepcl_hip_sys::hipEventDestroy(self.event);
    ...
    pub fn wait_sync(self) {
        unsafe {
            let status = deepcl_hip_sys::hipEventSynchronize(self.event);
            assert_eq!(
                status, HIP_SUCCESS,
                "Should successfully wait for stream event"
            );
