...
use deepcl_common::cache::Cache;
use deepcl_common::cache::CacheOption;
use deepcl_core::compute::CubeTask;
use deepcl_core::compute::DebugInformation;
use deepcl_core::prelude::*;
use deepcl_cpp::formatter::format_cpp;
use deepcl_cpp::shared::CompilationOptions;
use deepcl_hip_sys::{HIP_SUCCESS, get_hip_include_path, hiprtcResult_HIPRTC_SUCCESS};
use deepcl_runtime::logging::ServerLogger;
use deepcl_runtime::timestamp_profiler::TimestampProfiler;
...
pub struct HipCompiledKernel {
    _module: deepcl_hip_sys::hipModule_t,
    func: deepcl_hip_sys::hipFunction_t,
    cube_dim: CubeDim,
...
impl HipContext {
    pub fn new(compilation_options: CompilationOptions) -> Self {
        Self {
            ...
            compilation_options,
            compilation_cache: {
                let config = deepcl_runtime::config::GlobalConfig::get();
    ...
    pub fn compile_kernel(
        ...
        let program = unsafe {
            ...
            let mut program: deepcl_hip_sys::hiprtcProgram = std::ptr::null_mut();
            let status = deepcl_hip_sys::hiprtcCreateProgram(
                &mut program,
                source.as_ptr(),
        ...
        unsafe {
            ...
            let status =
                deepcl_hip_sys::hiprtcCompileProgram(program, options.len() as i32, options_ptr);
            if status != hiprtcResult_HIPRTC_SUCCESS {
                let mut log_size: usize = 0;
                let status =
                    deepcl_hip_sys::hiprtcGetProgramLogSize(program, &mut log_size as *mut usize);
                assert_eq!(
                    status, hiprtcResult_HIPRTC_SUCCESS,
                    "Should retrieve the compilation log size"
                );
                let mut log_buffer = vec![0; log_size];
                let status = deepcl_hip_sys::hiprtcGetProgramLog(program, log_buffer.as_mut_ptr());
                assert_eq!(
                    status, hiprtcResult_HIPRTC_SUCCESS,
        ...
        unsafe {
            let status = deepcl_hip_sys::hiprtcGetCodeSize(program, &mut code_size);
            assert_eq!(
                status, hiprtcResult_HIPRTC_SUCCESS,
        ...
        unsafe {
            let status = deepcl_hip_sys::hiprtcGetCode(program, code.as_mut_ptr());
            assert_eq!(
                status, hiprtcResult_HIPRTC_SUCCESS,
    ...
    fn load_compiled_binary(
        ...
        let mut module: deepcl_hip_sys::hipModule_t = std::ptr::null_mut();
        unsafe {
            let codeptr = code.as_ptr();
            let status = deepcl_hip_sys::hipModuleLoadData(&mut module, codeptr as *const _);
            assert_eq!(status, HIP_SUCCESS, "Should load compiled code into module");
        }
        // Retrieve the HIP module function
        let mut func: deepcl_hip_sys::hipFunction_t = std::ptr::null_mut();
        unsafe {
            let status =
                deepcl_hip_sys::hipModuleGetFunction(&mut func, module, func_name.as_ptr());
    ...
    pub fn execute_task(
        ...
        unsafe {
            let status = deepcl_hip_sys::hipModuleLaunchKernel(
                kernel.func,
                dispatch_count.0,
                dispatch_count.1,
                dispatch_count.2,
            ...
            if status == deepcl_hip_sys::hipError_t_hipErrorOutOfMemory {
