...
use deepcl_core::MemoryConfiguration;
...
#[derive(Debug)]
pub struct Stream {
    pub(crate) sys: deepcl_hip_sys::hipStream_t,
...
impl EventStreamBackend for HipStreamBackend {
    ...
    fn create_stream(&self) -> Self::Stream {
        let stream = unsafe {
            let mut stream: deepcl_hip_sys::hipStream_t = std::ptr::null_mut();
            let stream_status = deepcl_hip_sys::hipStreamCreate(&mut stream);
