...
fn create_client<M: DialectWmmaCompiler<HipDialect<M>>>(
    ...
    unsafe {
        ...
        let status = deepcl_hip_sys::hipGetDevicePropertiesR0600(
            ll_device_props.as_mut_ptr(),
            device.index as deepcl_hip_sys::hipDevice_t,
        );
    ...
    unsafe {
        let status = deepcl_hip_sys::hipSetDevice(device.index as deepcl_hip_sys::hipDevice_t);
        assert_eq!(
            status, HIP_SUCCESS,
    ...
    let max_memory = unsafe {
        let free: usize = 0;
        let total: usize = 0;
        let status = deepcl_hip_sys::hipMemGetInfo(
