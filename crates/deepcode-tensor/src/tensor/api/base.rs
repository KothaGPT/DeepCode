...
use deepcode_common::stub::RwLock;
    ...
    K::Elem: Element,
{
    ...
    /// ```rust
    /// use deepcode_tensor::backend::Backend;
    /// use deepcode_tensor::{Tensor, Shape};
    ///
    /// fn example<B: Backend>() {
    ...
    /// ```rust
    /// use deepcode_tensor::backend::Backend;
    /// use deepcode_tensor::{Tensor, Shape, s};
    ///
    /// fn example<B: Backend>() {
    ///     let device = B::Device::default();
    ///
    ///     // Single dimension slicing - no brackets needed!
    ///     let tensor = Tensor::<B, 1, deepcode_tensor::Int>::arange(0..10, &device);
    ...
    /// ```rust
    /// use deepcode_tensor::backend::Backend;
    /// use deepcode_tensor::{Tensor, Int};
    ///
    /// fn example<B: Backend>() {
    ///   let device = B::Device::default();
    ...
    /// ```rust
    /// use deepcode_tensor::backend::Backend;
    /// use deepcode_tensor::{Tensor, Bool};
    ///
    /// fn example<B: Backend>() {
    ///   let device = Default::default();
    ...
    <K as BasicOps<B>>::Elem: Debug,
{
    ...
    fn fmt_inner_tensor(
        ...
        for i in start..end {
            ...
            let data =
                deepcode_common::reader::try_read_sync(self.clone().slice(range).into_data_async());
