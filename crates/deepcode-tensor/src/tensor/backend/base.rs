...
/// This trait defines all types and functions needed for a backend to be used with deepcode.
///
/// ## Design
///
...
/// ### Eager Mode
///
/// Because deepcode supports dynamic graphs, the backend trait is designed around kernel
...
/// with deepcode. This pattern could also be used to create an operation fusion trait, which
...
/// For modules, public functions are often created, which can be used by `deepcode-core` modules.
pub trait Backend:
    FloatTensorOps<Self>
    + BoolTensorOps<Self>
    + IntTensorOps<Self>
