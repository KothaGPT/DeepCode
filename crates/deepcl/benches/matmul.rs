...
use deepcl::prelude::*;
use deepcl_matmul::AsyncReadingStrategy;
use deepcl_matmul::components::batch::HypercubeSelection;
use deepcl_matmul::components::stage::PartitionBuffering;
use deepcl_matmul::components::{
    LhsG, LoadingPrecomputeStrategy, MatmulElems, MatmulPrecision, MatmulSelection, RhsG,
    StageSize, TilingScheme,
};
use deepcl_matmul::kernels::layered::double_buffering::DoubleBufferingArgs;
use deepcl_matmul::kernels::layered::double_unit::DoubleUnitSelectionArgs;
use deepcl_matmul::kernels::layered::ordered_double_buffering::OrderedSelectionArgs;
use deepcl_matmul::kernels::layered::simple::SimpleArgs;
use deepcl_matmul::kernels::layered::simple_unit::SimpleUnitSelectionArgs;
use deepcl_matmul::kernels::layered::{Selection, TileSizeSelection};
use deepcl_matmul::{
...
use deepcl::benchmark::{Benchmark, BenchmarkComputations, BenchmarkDurations, TimingMethod};
use deepcl::future;
use deepcl_runtime::config::GlobalConfig;
use deepcl_std::tensor::TensorHandle;

use deepcl_random::random_uniform;
...
impl<R: Runtime, MP: MatmulPrecision> Benchmark for MatmulBench<R, MP> {
    ...
    fn profile(&self, args: Self::Input) -> Result<deepcl::benchmark::ProfileDuration, String> {
        self.client
        ...
        for p in [(1, 1, 1)] {
            for s in [(1, 1, 1)] {
                ...
                let hypercube = HypercubeSelection::builder(&tiling)
                    .global_order(deepcl_matmul::components::batch::GlobalOrderSelection::Default)
                    .cube_count_plan(
                        deepcl_matmul::components::batch::CubeCountPlanSelection::Flattened,
                    )
                    .build();
...
fn main() {
    ...
    {
        run_benches::<deepcl::wgpu::WgpuRuntime, f32>();
        // run_benches::<deepcl::wgpu::WgpuRuntime, half::f16>();
    }
    ...
    {
        // run_benches::<deepcl::hip::HipRuntime, f32>();
        run_benches::<deepcl::hip::HipRuntime, half::f16>();
    }

    #[cfg(feature = "cuda")]
    {
        // run_benches::<deepcl::cuda::CudaRuntime, f32>();
        run_benches::<deepcl::cuda::CudaRuntime, half::f16>();
        // run_benches::<deepcl::cuda::CudaRuntime, deepcl::flex32>();
        // run_benches::<deepcl::cuda::CudaRuntime, deepcl_std::SymQ8>();
        // run_benches::<deepcl::cuda::CudaRuntime, (i8, i8, i32, i32)>();
        // run_benches::<deepcl::cuda::CudaRuntime, (i8, i8, i32, i8)>();
        // run_benches::<deepcl::cuda::CudaRuntime, (i8, half::f16, half::f16, half::f16)>();
        // run_benches::<deepcl::cuda::CudaRuntime, (i8, half::bf16, f32, f32)>();
        // run_benches::<deepcl::cuda::CudaRuntime, (i8, half::f16, f32, half::f16)>();
    }
