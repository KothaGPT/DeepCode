...
use deepcode_communication::Address;
...
use deepcode::{backend::NdArray, prelude::Backend, tensor::Tensor};
use deepcode_collective::{AllReduceStrategy, ReduceOperation};
use deepcode_collective_multinode_tests::shared::{NodeTest, NodeTestResult, TENSOR_RANK};
use deepcode_common::rand::{SeedableRng, StdRng};
...
fn generate_random_input(
    ...
    let input: Vec<TensorData> = (0..input_count)
        .map(|_| {
            TensorData::random::<f32, _, _>(shape, deepcode::tensor::Distribution::Default, &mut rng)
        })
        .collect();
