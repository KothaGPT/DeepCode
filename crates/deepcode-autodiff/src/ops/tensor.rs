...
impl<B: Backend, C: CheckpointStrategy> FloatTensorOps<Self> for Autodiff<B, C> {
    ...
    fn float_random(
        shape: Shape,
        distribution: deepcode_tensor::Distribution,
        device: &Device<Self>,
    ) -> FloatTensor<Self> {
    ...
    fn float_slice(tensor: FloatTensor<Self>, slices: &[deepcode_tensor::Slice]) -> FloatTensor<Self> {
        #[derive(Debug)]
        struct Index;
        ...
        struct RetroSlice<B: Backend> {
            tensor_id: NodeId,
            slices: Vec<deepcode_tensor::Slice>,
            _backend: PhantomData<B>,
        }
        ...
        impl<B: Backend> Backward<B, 1> for Index {
            type State = (Vec<deepcode_tensor::Slice>, Shape, B::Device);
            ...
            fn step(self: Box<Self>, grads: &mut Gradients, _checkpointer: &mut Checkpointer) {
                ...
                self.nodes
                    ...
                    .for_each(|(node, dim_size)| {
                        ...
                        let slices: Vec<deepcode_tensor::Slice> = ranges
                            .iter()
                            .map(|r| {
                                deepcode_tensor::Slice::new(r.start as isize, Some(r.end as isize), 1)
                            })
                            .collect();
    ...
    fn float_cast(tensor: FloatTensor<Self>, dtype: deepcode_tensor::FloatDType) -> FloatTensor<Self> {
        AutodiffTensor::new(B::float_cast(tensor.primitive, dtype))
    }

    // TODO: Implement float_prod and float_sum
    // https://github.com/kothagpt/deepcode/issues/1458
