...
use deepcode_tensor::ops::unfold::{calculate_unfold_shape, calculate_unfold_windows};
use deepcode_tensor::{Element, Shape, TensorData, TensorMetadata, backend::Backend};
...
pub fn slice_with_steps(tensor: CandleTensor, slices: &[deepcode_tensor::Slice]) -> CandleTensor {
...
/// Generate indices for each dimension based on slice specifications
fn generate_slice_indices(slices: &[deepcode_tensor::Slice], tensor_dims: &[usize]) -> Vec<Vec<usize>> {
