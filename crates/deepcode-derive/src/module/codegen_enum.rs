...
impl ModuleCodegen for EnumModuleCodegen {
    ...
    fn gen_num_params(&self) -> TokenStream {
        let match_body = self.gen_variants_match_fn(|_| {
            quote! {
                deepcode::module::Module::<B>::num_params(module)
            }
        });
            ...
            quote! {
                {
                    ...
                    deepcode::module::Module::visit(module, visitor);
    ...
    fn gen_collect_devices(&self) -> TokenStream {
        let match_body = self.gen_variants_match_fn(|_| {
            quote! {
                deepcode::module::Module::<B>::collect_devices(module, devices)
            }
        });
    ...
    fn gen_to_device(&self) -> TokenStream {
        let match_body = self.gen_variants_match_fn(|variant| {
            quote! {
                Self::#variant(deepcode::module::Module::<B>::to_device(module, device))
            }
        });
    ...
    fn gen_fork(&self) -> TokenStream {
        let match_body = self.gen_variants_match_fn(|variant| {
            quote! {
                Self::#variant(deepcode::module::Module::<B>::fork(module, device))
            }
        });
            ...
            quote! {
                {
                    ...
                    let result = deepcode::module::Module::<B>::map(module, mapper);
    ...
    fn gen_valid(&self) -> TokenStream {
        let match_body = self.gen_variants_match_fn(|variant| {
            quote! {
                Self::InnerModule::#variant(deepcode::module::AutodiffModule::<B>::valid(module))
            }
        });
    ...
    fn gen_into_record(&self) -> TokenStream {
        let match_body = self.gen_variants_match_fn(|variant| {
            quote! {
                Self::Record::#variant(deepcode::module::Module::<B>::into_record(module))
            }
        });
            ...
            quote! {
                {
                    ...
                    Self::#variant(deepcode::module::Module::<B>::load_record(module, r))
                }
            }
        });
