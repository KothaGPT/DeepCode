...
pub(crate) fn generate_module_standard<Codegen: ModuleCodegen>(
    ...
    let mut codegen = quote! {
        impl #generics_module deepcode::module::Module<B> for #name #generics_ty_module #generics_where_module {
        ...
        impl #generics_module_autodiff deepcode::module::AutodiffModule<B> for #name #generics_ty_module_autodiff #generics_where_module_autodiff
        {
        ...
        impl #generics_module deepcode::module::ModuleDisplayDefault for #name #generics_ty_module #generics_where_module {
            #attributes_fn

            fn num_params(&self) -> usize {
                deepcode::module::Module::num_params(self)
            }
        }
    ...
    if !has_custom_display(&ast.attrs) {
        codegen.extend(quote! {
            impl #generics_module deepcode::module::ModuleDisplay for #name #generics_ty_module #generics_where_module {
...
pub(crate) fn generate_module_const(ast: &syn::DeriveInput) -> TokenStream {
    ...
    let backend: syn::Generics = parse_quote! { <B: deepcode::tensor::backend::Backend >};
    let backend_ad: syn::Generics = parse_quote! { <B: deepcode::tensor::backend::AutodiffBackend >};
    ...
    let mut codegen = quote! {
        impl #generics_module deepcode::module::Module<B> for #name #generics_ty #generics_where {
            deepcode::constant!(module);
        }

        impl #generics_module_ad deepcode::module::AutodiffModule<B>
            for #name #generics_ty #generics_where {
            deepcode::constant!(ad_module, #name #generics_ty);
        }
        ...
        impl #generics deepcode::module::ModuleDisplayDefault for #name #generics_ty #generics_where {
            #attributes_fn
        }
    ...
    if !has_custom_display(&ast.attrs) {
        codegen.extend(quote! {
            impl  #generics deepcode::module::ModuleDisplay for #name #generics_ty #generics_where {
...
impl GenericsParser {
    fn from_ast(generics: &Generics) -> Self {
        ...
        module_autodiff.add_predicate(parse_quote! {
                B: deepcode::tensor::backend::AutodiffBackend
        });

        module_autodiff.add_predicate(parse_quote! {
                <B as deepcode::tensor::backend::AutodiffBackend>::InnerBackend: #backend_trait
        });
        ...
        .for_each(|ident| {
            module.add_predicate(
                parse_quote! {
                    #ident: deepcode::module::Module<B>
                }
            );

            module.add_predicate(
                parse_quote! {
                    #ident: deepcode::module::ModuleDisplay
                }
            );

            module_autodiff.add_predicate(
                parse_quote! {
                    #ident: deepcode::module::AutodiffModule<B>
                }
            );

            module_autodiff.add_predicate(
                parse_quote! {
                    <#ident as deepcode::module::AutodiffModule<B>>::InnerModule: deepcode::module::Module<B::InnerBackend>
                }
            );

            module_autodiff.add_predicate(
                parse_quote! {
                    <#ident as deepcode::module::AutodiffModule<B>>::InnerModule: deepcode::module::ModuleDisplay
                }
            );

            generics_names_except_backend.extend(quote! { <#ident as deepcode::module::AutodiffModule<B>>::InnerModule, });
