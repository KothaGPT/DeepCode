...
use crate::{
    deepcode::{graph::DeepcodeGraph, node::try_convert_onnx_node},
    format_tokens,
    logger::init_log,
};
...
pub use crate::deepcode::graph::RecordType;

/// Builder for generating Deepcode model code from ONNX files.
///
/// `ModelGen` converts ONNX models into Deepcode-compatible Rust source code and model weights.
...
/// 2. Converts ONNX operations to Deepcode nodes using the node registry
...
///
/// ```no_run
/// use deepcode_import::onnx::{ModelGen, RecordType};
///
/// ModelGen::new()
///     .input("path/to/model.onnx")
///     .out_dir("src/model/")
...
impl ModelGen {
    ...
    /// - `<model>.graph.txt` - Debug representation of the converted Deepcode graph
    ///
    /// # Arguments
    ///
    ...
    fn run(&self, is_build_script: bool) {
        log::info!("Starting to convert ONNX to Deepcode");
        ...
        log::info!("Finished converting ONNX to Deepcode");
    }
    ...
    fn generate_model(&self, input: &PathBuf, out_file: PathBuf) {
        ...
        let top_comment = Some(format!("Generated from ONNX {input:?} by deepcode-import"));
    ...
    fn generate_code_with_precision(
        ...
        if self.double_precision {
            self.generate_deepcode_graph::<DoublePrecisionSettings>(
                graph,
                out_file,
                blank_space,
                top_comment,
            ...
            self.generate_deepcode_graph::<HalfPrecisionSettings>(
                graph,
                out_file,
                blank_space,
                top_comment,
            ...
            self.generate_deepcode_graph::<FullPrecisionSettings>(
                graph,
                out_file,
                blank_space,
                top_comment,
    ...
    /// Generate DeepcodeGraph with specified precision settings and codegen.
    fn generate_deepcode_graph<PS: PrecisionSettings + 'static>(
        &self,
        graph: ParsedOnnxGraph,
        out_file: &PathBuf,
        blank_space: bool,
...
impl ParsedOnnxGraph {
    /// Converts ONNX graph to Deepcode graph.
    pub fn into_deepcode<PS: PrecisionSettings + 'static>(self) -> DeepcodeGraph<PS> {
        let mut graph = DeepcodeGraph::<PS>::default();

        let mut unsupported_ops = vec![];

        for node in self.0.nodes {
            // Try registry-based conversion
            if let Some(deepcode_node) = try_convert_onnx_node::<PS>(node.clone()) {
                graph.register(deepcode_node);
            } else {
