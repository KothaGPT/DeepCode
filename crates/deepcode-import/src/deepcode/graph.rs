use super::{DeepcodeImports, Scope, Type};
use crate::deepcode::{
    TensorType,
    node::{Node, NodeCodegen},
};
use deepcode::record::{
    BinFileRecorder, DeepcodeRecord, FileRecorder, NamedMpkFileRecorder, NamedMpkGzFileRecorder,
...
pub enum RecordType {
    ...
    ///       See [#952 bug](https://github.com/kothagpt/deepcode/issues/952).
    NamedMpkGz,
...
/// Deepcode graph intermediate representation of modules and tensor operations.
#[derive(Default, Debug)]
pub struct DeepcodeGraph<PS: PrecisionSettings> {
    nodes: Vec<Node<PS>>,
    scope: Scope,
    imports: DeepcodeImports,
...
type Backend = deepcode_ndarray::NdArray;

impl<PS: PrecisionSettings> DeepcodeGraph<PS> {
    ...
    pub fn with_record(
        ...
        self.imports
            .register(format!("deepcode::record::{precision_ty_str}"));
        ...
        match record_type {
            RecordType::PrettyJson => {
                ...
                Recorder::<Backend>::save_item(
                    &recorder,
                    DeepcodeRecord::<_, Backend>::new::<PrettyJsonFileRecorder<PS>>(StructMap(
                        DeepcodeGraphState::new(&self.nodes),
                    )),
                    out_file.clone(),
                )
                .unwrap();
                ...
                self.register_record_file(
                    out_file,
                    &format!("deepcode::record::PrettyJsonFileRecorder::<{precision_ty_str}>"),
                );
            }
            RecordType::NamedMpkGz => {
                ...
                Recorder::<Backend>::save_item(
                    &recorder,
                    DeepcodeRecord::<_, Backend>::new::<NamedMpkGzFileRecorder<PS>>(StructMap(
                        DeepcodeGraphState::new(&self.nodes),
                    )),
                    out_file.clone(),
                )
                ...
                self.register_record_file(
                    out_file,
                    &format!("deepcode::record::NamedMpkGzFileRecorder::<{precision_ty_str}>"),
                );
            }
            ...
            RecordType::NamedMpk => {
                ...
                self.register_record_file(
                    out_file,
                    &format!("deepcode::record::NamedMpkFileRecorder::<{precision_ty_str}>"),
                );
            }
            ...
            RecordType::Bincode => {
                ...
                Recorder::<Backend>::save_item(
                    &recorder,
                    DeepcodeRecord::<_, Backend>::new::<BinFileRecorder<PS>>(StructTuple(
                        DeepcodeGraphState::new(&self.nodes),
                    )),
                    out_file.clone(),
                )
                ...
                if embed_states {
                    ...
                    self.register_record_file(
                        out_file,
                        &format!("deepcode::record::BinFileRecorder::<{precision_ty_str}>"),
                    );
                }
            }
        }
    ...
    /// Generate tokens representing the graph with Deepcode modules and tensor operations.
    pub fn codegen(mut self) -> TokenStream {
        ...
        quote! {
            // @generated
            // This file is automatically generated by deepcode-import
    ...
    fn register_record_file(&mut self, file: PathBuf, recorder_str: &str) {
        self.imports.register("deepcode::record::Recorder");
    ...
    fn register_record_embed(&mut self, file: PathBuf) {
        ...
        self.imports.register("deepcode::record::BinBytesRecorder");
...
#[derive(new, Debug)]
struct DeepcodeGraphState<'a, PS: PrecisionSettings> {
    nodes: &'a Vec<Node<PS>>,
}
...
struct StructMap<'a, PS: PrecisionSettings>(DeepcodeGraphState<'a, PS>);
...
struct StructTuple<'a, PS: PrecisionSettings>(DeepcodeGraphState<'a, PS>);
