...
use crate::deepcode::{Scope, TensorKind, ToTokens, Type};
        ...
        let raw_output_expr = match self.reduction_type {
                ...
                match &self.input {
                    Type::Tensor(tensor) => match tensor.kind {
                        TensorKind::Int => {
                            ...
                            quote! { #input_square_reduced.float().cast(deepcode::tensor::DType::F32).sqrt().int() }
                        }
                        TensorKind::Float => {
                            // Cast to F32 before sqrt to avoid overflow/underflow in lower precision types,
                            ...
                            quote! {
                                ...
                                #input_square_reduced.cast(deepcode::tensor::DType::F32).sqrt().cast(input_dtype)
                            }
                        }
            ...
            ReductionType::LogSum => {
                ...
                match &self.input {
                    Type::Tensor(tensor) => match tensor.kind {
                        TensorKind::Int => {
                            quote! { #input_reduced.float().cast(deepcode::tensor::DType::F32).log().int() }
                        }
                        TensorKind::Float => {
                            quote! {
                                let input_dtype = #input.dtype();
                                #input_reduced.cast(deepcode::tensor::DType::F32).log().cast(input_dtype)
                            }
                        }
            ...
            ReductionType::LogSumExp => {
                let input_double = match &self.input {
                    Type::Tensor(tensor) => match tensor.kind {
                        TensorKind::Int => {
                            quote! { #input.float().cast(deepcode::tensor::DType::F32) }
                        }
                        TensorKind::Float => {
                            quote! { #input.cast(deepcode::tensor::DType::F32) }
                        }
                ...
                pub fn forward(&self, tensor1: Tensor<B, 4>) -> Tensor<B, 2> {
                    let tensor2 = {
                        ...
                        tensor1
                            ...
                            .sum_dim(3usize)
                            .squeeze_dims(&[0, 3])
                            .cast(deepcode::tensor::DType::F32)
                            .sqrt()
                            .cast(input_dtype)
                    };
            ...
            impl<B: Backend> Model <B> {
                ...
                pub fn forward(&self, tensor1: Tensor<B, 4>) -> Tensor<B, 4> {
                    let tensor2 = {
                        ...
                        let input_double = tensor1.cast(deepcode::tensor::DType::F32);
