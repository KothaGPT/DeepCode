...
use crate::deepcode::{ScalarKind, Scope, TensorKind, Type};
    ...
    fn forward(&self, scope: &mut Scope, node_position: usize) -> TokenStream {
        ...
        match (&self.input, &self.output) {
            ...
            (Type::Scalar(input_scalar), Type::Scalar(output_scalar)) => {
                ...
                // Map ONNX element types to Deepcode ScalarKind "families".
                // Deepcode scalar kinds are coarse (Float32/Float64/Int64/Bool), so:
            ...
            (Type::Tensor(input_tensor), Type::Tensor(output_tensor)) => {
                ...
                // Map ONNX element types to Deepcode TensorKind categories.
                // Deepcode only distinguishes Float / Int / Bool at the Tensor level.
                let target_kind = match self.target_elem_type {
                ...
                if input_tensor.kind == target_kind {
                    ...
                    // Deepcode exposes category-level casts: .float(), .int(), .bool()
...
impl OnnxIntoNode for CastNode {
    fn from_onnx(node: onnx_ir::Node) -> Self {
        let input = crate::deepcode::Type::from(node.inputs.first().unwrap());
        let output = crate::deepcode::Type::from(node.outputs.first().unwrap());
...
mod tests {
    use super::*;
    use crate::deepcode::{
        ScalarKind, ScalarType, TensorType, graph::DeepcodeGraph, node::test::assert_tokens,
    };
