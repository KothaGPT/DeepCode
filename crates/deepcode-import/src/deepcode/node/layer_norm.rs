...
impl<PS: PrecisionSettings> NodeCodegen<PS> for LayerNormNode {
        ...
        if self.full_precision {
            quote! {
                let #output = {
                    ...
                    self.#field.forward(#input.cast(deepcode::tensor::DType::F32)).cast(dtype)
                };
            }
        } else {
            quote! {
    ...
    fn register_imports(&self, imports: &mut DeepcodeImports) {
        imports.register("deepcode::nn::LayerNorm");
        imports.register("deepcode::nn::LayerNormConfig");
    }
...
mod tests {
    ...
    fn test_codegen() {
        ...
        let expected = quote! {
            ...
            use deepcode::nn::LayerNorm;
            use deepcode::nn::LayerNormConfig;
            ...
            impl<B: Backend> Model <B> {
                ...
                pub fn forward(&self, input: Tensor<B, 4>) -> Tensor<B, 4> {
                    let output = {
                        let dtype = input.dtype();
                        self.norm
                            .forward(input.cast(deepcode::tensor::DType::F32))
                            .cast(dtype)
                    };
