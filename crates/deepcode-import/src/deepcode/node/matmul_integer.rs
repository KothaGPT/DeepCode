...
mod tests {
    use super::*;
    use crate::deepcode::{TensorType, graph::DeepcodeGraph};
    use deepcode::record::FullPrecisionSettings;
    use quote::quote;

    #[test]
    fn codegen_basic_no_zp() {
        let mut g = DeepcodeGraph::<FullPrecisionSettings>::default();
        ...
        let _expected = quote! {
            ...
            impl<B: Backend> Model<B> {
                pub fn new(device: &B::Device) -> Self {
                    Self { phantom: core::marker::PhantomData, device: deepcode::module::Ignored(device.clone()) }
                }
