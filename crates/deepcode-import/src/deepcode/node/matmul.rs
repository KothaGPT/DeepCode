...
use crate::deepcode::{Scope, TensorKind, TensorType, Type};
...
impl OnnxIntoNode for MatmulNode {
    fn from_onnx(node: onnx_ir::Node) -> Self {
        let lhs = crate::deepcode::TensorType::from(node.inputs.first().unwrap());
        let rhs = crate::deepcode::TensorType::from(node.inputs.get(1).unwrap());
        let output = crate::deepcode::TensorType::from(node.outputs.first().unwrap());
