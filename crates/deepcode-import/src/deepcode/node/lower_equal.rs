    ...
    fn forward(&self, scope: &mut Scope, node_position: usize) -> TokenStream {
        ...
        let function = match (&self.lhs, &self.rhs) {
            ...
            (Type::Shape(_), Type::Tensor(_)) => quote! {
                Tensor::<B, 1, deepcode::tensor::Int>::from_data(&#lhs as &[_], &*self.device).lower_equal(#rhs)
            },
            (Type::Tensor(_), Type::Shape(_)) => quote! {
                #lhs.lower_equal(Tensor::<B, 1, deepcode::tensor::Int>::from_data(&#rhs as &[_], &*self.device))
            },
