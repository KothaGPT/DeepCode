...
use crate::deepcode::{Scope, ToTokens, Type};
        ...
        match (&self.input, &self.output) {
            ...
            (Type::Shape(input), Type::Scalar(output)) => {
                ...
                let cast_expr = match &output.kind {
                    crate::deepcode::ScalarKind::Int64 => quote! { #input_name[0] as i64 },
                    crate::deepcode::ScalarKind::Int32 => quote! { #input_name[0] as i32 },
                    _ => panic!(
            ...
            (Type::Tensor(input), Type::Scalar(output)) => {
                ...
                let elem_cast = match &output.kind {
                    crate::deepcode::ScalarKind::Float32 => quote! { .elem::<f32>() },
                    crate::deepcode::ScalarKind::Float64 => quote! { .elem::<f64>() },
                    crate::deepcode::ScalarKind::Int32 => quote! { .elem::<i32>() },
                    crate::deepcode::ScalarKind::Int64 => quote! { .elem::<i64>() },
                    crate::deepcode::ScalarKind::Bool => quote! { .elem::<bool>() },
                };
    ...
    fn test_squeeze_shape_to_scalar() {
        ...
        graph.register(SqueezeNode::new(
            ...
            Type::Scalar(ScalarType::new(
                "scalar1",
                crate::deepcode::ty::ScalarKind::Int64,
            )),
            Some(vec![0]),
        ));
