...
use crate::deepcode::{OtherType, Scope, TensorType, ToTokens, Type};
...
impl ResizeMode {
    ...
    pub fn to_tensor_interpolate_mode_token(&self) -> TokenStream {
        match self {
            ResizeMode::Nearest => quote! { deepcode::tensor::ops::InterpolateMode::Nearest },
            ResizeMode::Linear => quote! { deepcode::tensor::ops::InterpolateMode::Bilinear },
            ResizeMode::Cubic => quote! { deepcode::tensor::ops::InterpolateMode::Bicubic },
        }
    }
}
            ...
            Some(ResizeSizes::Runtime(Type::Shape(shape))) => {
                ...
                if self.input.rank == 4 {
                    ...
                    quote! {
                        ...
                        let #output = deepcode::tensor::module::interpolate(
                            #input,
                            [target_height, target_width],
                            deepcode::tensor::ops::InterpolateOptions::new(#mode_token)
                        );
                    }
                } else {
                    panic!(
...
impl<PS: PrecisionSettings> NodeCodegen<PS> for ResizeNode {
    ...
    fn register_imports(&self, imports: &mut crate::deepcode::DeepcodeImports) {
        if self.field.is_some() {
            ...
            imports.register("deepcode::nn::interpolate::InterpolateMode");
            if self.input.rank == 3 {
                imports.register("deepcode::nn::interpolate::Interpolate1dConfig");
                imports.register("deepcode::nn::interpolate::Interpolate1d");
            } else if self.input.rank == 4 {
                imports.register("deepcode::nn::interpolate::Interpolate2dConfig");
                imports.register("deepcode::nn::interpolate::Interpolate2d");
            } else {
                panic!("Unsupported input rank for resize node");
            }
        }
...
impl OnnxIntoNode for ResizeNode {
    fn from_onnx(node: onnx_ir::Node) -> Self {
        ...
        // Convert from onnx-ir types to deepcode types
...
mod tests {
    ...
    fn test_codegen_nodes_2d() {
        ...
        let expected = quote! {
            ...
            use deepcode::nn::interpolate::Interpolate2d;
            use deepcode::nn::interpolate::Interpolate2dConfig;
            use deepcode::nn::interpolate::InterpolateMode;
            #[derive(Module, Debug)]
    ...
    fn test_codegen_nodes_1d() {
        ...
        let expected = quote! {
            use deepcode::prelude::*;
            use deepcode::nn::interpolate::Interpolate1d;
            use deepcode::nn::interpolate::Interpolate1dConfig;
            use deepcode::nn::interpolate::InterpolateMode;
            #[derive(Module, Debug)]
            pub struct Model<B: Backend> {
