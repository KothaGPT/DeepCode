...
use crate::deepcode::{ScalarType, ShapeType, Type};
...
impl<PS: PrecisionSettings> NodeCodegen<PS> for WhereNode {
    ...
    fn forward(&self, scope: &mut crate::deepcode::Scope, node_position: usize) -> TokenStream {
        match &self.output {
            Type::Tensor(out) => {
        ...
        let expected = quote! {
            ...
            impl<B: Backend> Model <B> {
                ...
                pub fn forward(
                    ...
                    let tensor4 = Tensor::<B, 2, deepcode::tensor::Float>::full([1, 1], scalar3, &*self.device)
