...
impl ScalarType {
    ...
    pub fn to_full_tensor(&self, shape: &[usize]) -> TokenStream {
        ...
        let tensor_kind = match self.kind {
            ScalarKind::Int32 | ScalarKind::Int64 => quote! { deepcode::tensor::Int },
            ScalarKind::Float32 | ScalarKind::Float64 => quote! { deepcode::tensor::Float },
            ScalarKind::Bool => quote! { deepcode::tensor::Bool },
        };
        quote! {
...
impl ShapeType {
    ...
    pub fn to_tensor(&self) -> TokenStream {
        ...
        quote! { Tensor::<B, 1, deepcode::tensor::Int>::from_data(&#shape_name as &[_], &*self.device) }
    }
}
...
impl From<&onnx_ir::ir::Argument> for TensorType {
    fn from(arg: &onnx_ir::ir::Argument) -> Self {
        ...
        match &arg.ty {
            ...
            ArgType::Shape(_) => panic!("Cannot convert Shape to Deepcode TensorType"),
        }
    }
}
...
fn tensor_type_from_elem_and_rank(
    ...
    match elem {
        ...
        ElementType::String => {
            panic!("String element type cannot be converted to Deepcode TensorType")
        }
    }
}
