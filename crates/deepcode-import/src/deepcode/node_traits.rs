...
use crate::deepcode::{DeepcodeImports, Scope, Type};

pub type SerializationBackend = deepcode_ndarray::NdArray<f32>;

/// Trait for converting ONNX IR nodes to Deepcode nodes
#[allow(dead_code)]
pub trait OnnxIntoNode: Sized {
    /// Convert an ONNX IR node into this Deepcode node type
...
pub trait NodeCodegen<PS: PrecisionSettings>: std::fmt::Debug {
    ...
    fn register_imports(&self, _imports: &mut DeepcodeImports) {}
...
pub fn extract_node_data<E: deepcode::tensor::Element>(
    node: &onnx_ir::Node,
    input_index: usize,
) -> Option<deepcode::tensor::TensorData> {
