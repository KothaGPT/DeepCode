...
use deepcode_store::pytorch::PytorchReader;
use regex::Regex;
...
pub enum Error {
    #[error("Store error: {0}")]
    Store(#[from] deepcode_store::pytorch::PytorchError),

    #[error("Serde error: {0}")]
    Serde(#[from] deepcode::record::serde::error::Error),

    #[error("Other error: {0}")]
    Other(String),
}

/// Deserializes tensor data from a PyTorch file (`.pt` or `.pth`) into a Deepcode record.
///
/// This function reads tensors from a pickle file using deepcode-store's PyTorch reader,
...
/// * `D` - The target Deepcode record type to deserialize into.
...
where
    ...
    B: Backend,
{
    // Use deepcode-store's PyTorch reader to load tensors
...
// Re-export deepcode-store's PyTorch reader types for convenience

// Implement conversion to RecorderError for compatibility with the Recorder trait
impl From<Error> for deepcode::record::RecorderError {
    fn from(error: Error) -> Self {
        deepcode::record::RecorderError::DeserializeError(error.to_string())
    }
}
