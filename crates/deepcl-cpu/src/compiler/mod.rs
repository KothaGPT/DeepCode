...
use deepcl_opt::OptimizerBuilder;
...
impl Compiler for MlirCompiler {
    ...
    fn compile(
        ...
        MlirEngine::from_deepcl_ir(kernel, &opt, shared_memories)
    }
...
#[cfg(feature = "mlir-dump")]
fn dump_scope(scope: &deepcl_core::prelude::Scope, name: &str) {
    use std::fs;

    if let Ok(dir) = std::env::var("DEEPCL_DEBUG_MLIR") {
        let path = format!("{dir}/{name}");
        let _ = fs::create_dir(&path);
        fs::write(format!("{path}/deepcl.ir.txt"), format!("{}", scope)).unwrap();
    }
}

#[cfg(feature = "mlir-dump")]
fn dump_opt(opt: &deepcl_opt::Optimizer, name: &str) {
    if let Ok(dir) = std::env::var("DEEPCL_DEBUG_MLIR") {
        use std::fs;
        let path = format!("{dir}/{name}");
        let _ = fs::create_dir(&path);
        fs::write(format!("{path}/deepcl-opt.ir.txt"), format!("{}", opt)).unwrap();
        fs::write(
            format!("{path}/deepcl-opt.ir.dot"),
            format!("{}", opt.dot_viz()),
        )
        .unwrap();
    }
