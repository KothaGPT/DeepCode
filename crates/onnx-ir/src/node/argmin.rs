        ...
        match key.as_str() {
            ...
            "select_last_index" => {
                if value.clone().into_i64() != 0 {
                    panic!(
                        "select_last_index=1 is not supported for argmin in deepcode (got {value:?})"
                    );
                }
            }
            "keepdims" => {
                ...
                if keepdims_val != 0 && keepdims_val != 1 {
                    panic!(
                        "Only keepdims=0 or keepdims=1 is supported for argmin in deepcode (got {value:?})",
                    );
                }
...
pub fn argmin_update_outputs(node: &mut Node) {
    ...
    // For deepcode compatibility, argmin always outputs a tensor
...
mod tests {
    ...
    #[test]
    #[should_panic(expected = "Only keepdims=0 or keepdims=1 is supported for argmin in deepcode")]
    ...
    #[test]
    #[should_panic(expected = "select_last_index=1 is not supported for argmin in deepcode")]
    ...
    fn test_argmin_update_outputs_keepdims_0() {
        // Test argmin with keepdims=0 - output rank should be reduced but minimum 1 for deepcode
