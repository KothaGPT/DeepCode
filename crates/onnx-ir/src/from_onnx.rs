    ...
    fn handle_identity(&mut self, node: &mut Node) {
        if node.node_type == NodeType::Identity {
            ...
            if let Some(value) = &node.inputs[0].value {
                ...
                // deepcode-import and let it handle them.
                log::debug!(
                    "Identity node without constant - will pass through to deepcode-import: {}",
                    &node.name
                );
            }
        }
...
pub fn parse_onnx(onnx_path: &Path) -> OnnxGraph {
    ...
    if !verify_opsets(&onnx_model.opset_import, MIN_OPSET_VERSION) {
        panic!(
            ...
            See documentation (https://deepcode.dev/books/deepcode/import/onnx-model.html) for details."
        );
    }
