...
use deepcl_common::ExecutionMode;
use deepcl_core::CubeDim;
use deepcl_core::ir::{FloatKind, Processor, UIntKind, VariableKind};
use deepcl_core::post_processing::checked_io::CheckedIoProcessor;
use deepcl_core::{
    Compiler,
    ir::{self as gpu},
};
...
use deepcl_opt::{Optimizer, SharedLiveness};
use deepcl_runtime::{DeviceProperties, TypeUsage};
...
impl<D: Dialect> CppCompiler<D> {
    ...
    fn build_metadata(&mut self, value: &KernelDefinition) {
        ...
        self.metadata = deepcl_core::Metadata::new(num_meta as u32, num_ext);
    }
    ...
    fn compile_variable(&mut self, value: gpu::Variable) -> Variable<D> {
        ...
        match value.kind {
            ...
            gpu::VariableKind::Builtin(builtin) => match builtin {
                ...
                gpu::Builtin::DeepClusterDim => const_u32(self.cluster_dim.num_elems()),
                gpu::Builtin::DeepClusterDimX => const_u32(self.cluster_dim.x),
                gpu::Builtin::DeepClusterDimY => const_u32(self.cluster_dim.y),
                gpu::Builtin::DeepClusterDimZ => const_u32(self.cluster_dim.z),
                gpu::Builtin::CubePos => {
    ...
    fn compile_binding(&mut self, binding: deepcl_core::compute::Binding) -> Binding<D> {
        Binding {
            id: binding.id,
