...
use deepcode_tensor::DType;
use deepcode_tensor::quantization::{QParamTensor, QuantParam, QuantScheme};
use deepcl::prelude::{TensorArg, TensorHandleRef};
use deepcl::{CubeElement, Runtime};
use deepcl::{client::ComputeClient, ir::ElemType};
use serde::{Deserialize, Serialize};

/// Fusion optimization type for deepcl.
///
...
impl<R: Runtime> deepcode_fusion::NumOperations for CubeOptimization<R> {
    fn len(&self) -> usize {
        match self {
...
/// Fusion optimization state type for deepcl.
///
/// More optimization variants should be added here.
#[allow(clippy::large_enum_variant)]
...
/// tensor handle.
pub type QParams = deepcode_tensor::quantization::QParams<QParamTensor>;
...
pub struct CubeFusionHandle<R: Runtime> {
    ...
    pub handle: deepcl::server::Handle,
