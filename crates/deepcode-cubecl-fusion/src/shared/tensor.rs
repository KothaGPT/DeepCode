    ...
    pub fn __expand_read_method<C: CubePrimitive>(
        ...
        match self {
            GlobalScalarExpand::U64(val) => {
                if dtype == ElemType::UInt(deepcl::ir::UIntKind::U64) {
            ...
            GlobalScalarExpand::U32(val) => {
                if dtype == ElemType::UInt(deepcl::ir::UIntKind::U32) {
                    let expand: ExpandElement = val.clone().into();
                    ExpandElementTyped::from(expand.clone())
                } else {
                    C::__expand_cast_from(scope, val.clone())
            ...
            GlobalScalarExpand::U16(val) => {
                if dtype == ElemType::UInt(deepcl::ir::UIntKind::U16) {
                    let expand: ExpandElement = val.clone().into();
                    ExpandElementTyped::from(expand.clone())
                } else {
                    C::__expand_cast_from(scope, val.clone())
            ...
            GlobalScalarExpand::F64(val) => {
                if dtype == ElemType::Float(deepcl::ir::FloatKind::F64) {
                    let expand: ExpandElement = val.clone().into();
                    ExpandElementTyped::from(expand.clone())
                } else {
                    C::__expand_cast_from(scope, val.clone())
            ...
            GlobalScalarExpand::F32(val) => {
                if dtype == ElemType::Float(deepcl::ir::FloatKind::F32) {
                    let expand: ExpandElement = val.clone().into();
                    ExpandElementTyped::from(expand.clone())
                } else {
                    C::__expand_cast_from(scope, val.clone())
            ...
            GlobalScalarExpand::F16(val) => {
                if dtype == ElemType::Float(deepcl::ir::FloatKind::F16) {
                    let expand: ExpandElement = val.clone().into();
                    ExpandElementTyped::from(expand.clone())
                } else {
                    C::__expand_cast_from(scope, val.clone())
            ...
            GlobalScalarExpand::BF16(val) => {
                if dtype == ElemType::Float(deepcl::ir::FloatKind::BF16) {
                    let expand: ExpandElement = val.clone().into();
                    ExpandElementTyped::from(expand.clone())
                } else {
                    C::__expand_cast_from(scope, val.clone())
            ...
            GlobalScalarExpand::U8(val) => {
                if dtype == ElemType::UInt(deepcl::ir::UIntKind::U8) {
                    let expand: ExpandElement = val.clone().into();
                    ExpandElementTyped::from(expand.clone())
                } else {
                    C::__expand_cast_from(scope, val.clone())
            ...
            GlobalScalarExpand::I64(val) => {
                if dtype == ElemType::Int(deepcl::ir::IntKind::I64) {
                    let expand: ExpandElement = val.clone().into();
                    ExpandElementTyped::from(expand.clone())
                } else {
                    C::__expand_cast_from(scope, val.clone())
            ...
            GlobalScalarExpand::I32(val) => {
                if dtype == ElemType::Int(deepcl::ir::IntKind::I32) {
                    let expand: ExpandElement = val.clone().into();
                    ExpandElementTyped::from(expand.clone())
                } else {
                    C::__expand_cast_from(scope, val.clone())
            ...
            GlobalScalarExpand::I16(val) => {
                if dtype == ElemType::Int(deepcl::ir::IntKind::I16) {
                    let expand: ExpandElement = val.clone().into();
                    ExpandElementTyped::from(expand.clone())
                } else {
                    C::__expand_cast_from(scope, val.clone())
            ...
            GlobalScalarExpand::I8(val) => {
                if dtype == ElemType::Int(deepcl::ir::IntKind::I8) {
                    let expand: ExpandElement = val.clone().into();
                    ExpandElementTyped::from(expand.clone())
                } else {
                    C::__expand_cast_from(scope, val.clone())
