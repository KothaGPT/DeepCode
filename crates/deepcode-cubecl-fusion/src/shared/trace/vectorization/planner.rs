...
use deepcl_quant::scheme::{QuantScheme, QuantStore, QuantValue};
        ...
        for input in plan.handle_inputs.iter() {
            let elem: StorageType = match input {
                ...
                HandleInput::QuantValues(handle) => match handle.global_ir.dtype {
                    deepcode_tensor::DType::QFloat(scheme) => {
    ...
    match scheme.store {
        QuantStore::Native => match scheme.value {
            ...
            | QuantValue::E2M1 => {
                let line_sizes = R::io_optimized_line_sizes_unchecked(
                    &ElemType::Int(deepcl::ir::IntKind::I8).into(),
                )
                .collect::<Vec<u8>>();
        ...
        QuantStore::U32 => {
            let mut line_sizes = R::io_optimized_line_sizes_unchecked(
                &ElemType::Int(deepcl::ir::IntKind::I32).into(),
            )
            .collect::<Vec<u8>>();
