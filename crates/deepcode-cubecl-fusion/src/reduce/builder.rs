...
use deepcode_fusion::{OptimizationBuilder, OptimizationStatus};
use deepcode_ir::{NumericOperationIr, OperationIr, ReduceDimOpIr};
use deepcl::{Runtime, reduce::ReduceStrategy};
...
impl<R: Runtime> OptimizationBuilder<CubeOptimization<R>> for ReduceBuilder<R> {
    ...
    fn status(&self) -> deepcode_fusion::OptimizationStatus {
        self.builder.status()
    }

    fn properties(&self) -> deepcode_fusion::OptimizationProperties {
