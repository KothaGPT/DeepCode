use deepcode_common::future::DynFut;
use deepcode_communication::{Address, ProtocolClient, data_service::TensorTransferId};
use deepcode_ir::TensorIr;
use deepcode_router::{MultiBackendBridge, RouterTensor, RunnerClient, get_client};
use deepcode_tensor::{
    Shape, TensorData,
...
impl RunnerClient for RemoteClient {
    ...
    fn register(&self, op: deepcode_ir::OperationIr) {
        self.sender
            .send(ComputeTask::RegisterOperation(Box::new(op)));
    }

    fn read_tensor(&self, tensor: deepcode_ir::TensorIr) -> DynFut<TensorData> {
    ...
    fn register_empty_tensor(&self, shape: Shape, dtype: deepcode_tensor::DType) -> RouterTensor<Self> {
        let id = self.sender.new_tensor_id();
    ...
    fn register_float_tensor(
        &self,
        shape: Shape,
        dtype: deepcode_tensor::FloatDType,
    ) -> RouterTensor<Self> {
...
impl Default for RemoteDevice {
    fn default() -> Self {
        let address = match std::env::var("BDEEPCODE_REMOTE_ADDRESS") {
            Ok(address) => address,
...
impl deepcode_common::device::Device for RemoteDevice {
