...
mod tests {
    ...
    fn create_test_tensor_snapshot(name: &str) -> TensorSnapshot {
        let data = TensorData {
            bytes: deepcode_tensor::Bytes::from_bytes_vec(vec![1, 2, 3, 4]),
            shape: vec![2, 2],
            dtype: deepcode_tensor::DType::F32,
        };
    ...
    fn test_key_remapper_from_patterns() {
        let patterns = vec![(r"^pytorch\.", "deepcode."), (r"\.bias$", ".bias_param")];
        ...
        assert!(
            remapped
                .iter()
                .any(|v| v.full_path() == "deepcode.linear.bias_param")
        );
    }
