//! SafeTensors format support for Deepcode deep learning framework.
//!
...
//!
//! ```rust,ignore
//! use deepcode_store::safetensors::SafetensorsStore;
//! use deepcode_store::ModuleSnapshot;
//!
...
//!     .add_pattern(r"^pytorch\.(.*)", "deepcode.$1")?           // pytorch.layer -> deepcode.layer
...
//!
//! ```rust,ignore
//! use deepcode_store::{ModuleSnapshot, PyTorchToDeepcodeAdapter, DeepcodeToPyTorchAdapter};
//! use deepcode_store::safetensors::SafetensorsStore;
//!
//! // Loading PyTorch model into Deepcode
//! let mut store = SafetensorsStore::from_file("pytorch_model.safetensors")
//!     .with_from_adapter(PyTorchToDeepcodeAdapter)  // Transposes linear weights, renames norm params
//!     .allow_partial(true);                     // PyTorch models may have extra tensors
//!
//! let mut deepcode_model = Model::new(&device);
//! deepcode_model.apply_from(&mut store)?;
//!
//! // Saving Deepcode model for PyTorch
//! let mut store = SafetensorsStore::from_file("for_pytorch.safetensors")
//!     .with_to_adapter(DeepcodeToPyTorchAdapter);   // Transposes weights back, renames for PyTorch
//!
...
//! ## Adapter Methods
//!
//! - **`with_from_adapter(adapter)`** - Set adapter for loading (e.g., PyTorchToDeepcodeAdapter)
//! - **`with_to_adapter(adapter)`** - Set adapter for saving (e.g., DeepcodeToPyTorchAdapter)
//!
...
//!
//! Migrating a PyTorch model to Deepcode with filtering, remapping, and adapters:
//!
//! ```rust,ignore
//! use deepcode_store::{ModuleSnapshot, PyTorchToDeepcodeAdapter};
//! use deepcode_store::safetensors::SafetensorsStore;
//!
