    ...
    pub fn new(device: &B::Device) -> Self {
        Self {
            ...
            bool_tensor: Param::initialized(
                ParamId::new(),
                Tensor::from_bool(
                    deepcode::tensor::TensorData::new(
...
mod tests {
    ...
    fn test_mixed_dtypes_round_trip() {
        type TestBackend = deepcode_ndarray::NdArray<f32>;
        ...
        for snapshot in snapshots {
            ...
            if path.contains("float_tensor") || path.contains("linear") {
                ...
                assert!(
                    matches!(
                        dtype,
                        deepcode::tensor::DType::I64
                            | deepcode::tensor::DType::I32
                            | deepcode::tensor::DType::I16
                            | deepcode::tensor::DType::I8
                    ),
    ...
    fn test_mixed_precision_floats() {
        ...
        // (F16, BF16, F32, F64, etc.) in the same file, Deepcode's backend architecture currently
        ...
        {
            type TestBackend = deepcode_ndarray::NdArray<f64>;
            let device = Default::default();
    ...
    fn test_mixed_precision_integers() {
        ...
        struct MultiIntModel<B: Backend> {
            // Note: Deepcode's Tensor<B, D, Int> uses the backend's default int type
    ...
    fn test_comprehensive_mixed_types() {
        ...
        let mut loaded_model = ComprehensiveModel::<TestBackend> {
            ...
            boolean_mask: Param::initialized(
                ParamId::new(),
                Tensor::from_bool(
                    deepcode::tensor::TensorData::new(vec![false; 8], [2, 4]),
                    &device,
                ),
            ),
        };
