use crate::{DeepcodeToPyTorchAdapter, ModuleSnapshot, PyTorchToDeepcodeAdapter, SafetensorsStore};
use deepcode::nn::{Linear, LinearConfig};
use deepcode_core::module::{Module, Param};
use deepcode_tensor::Tensor;
use deepcode_tensor::backend::Backend;

type TestBackend = deepcode_ndarray::NdArray;
...
#[test]
fn pytorch_to_deepcode_adapter_linear_transpose() {
    let device = Default::default();
    let model = TestModel::<TestBackend>::new(&device);

    // Save with DeepcodeToPyTorch adapter (will transpose linear weights)
    let mut save_store = SafetensorsStore::from_bytes(None).with_to_adapter(DeepcodeToPyTorchAdapter);
    model.collect_to(&mut save_store).unwrap();

    // Load with PyTorchToDeepcode adapter (will transpose back)
    let mut load_store = SafetensorsStore::from_bytes(None).with_from_adapter(PyTorchToDeepcodeAdapter);
...
#[test]
fn pytorch_to_deepcode_adapter_norm_rename() {
    let device = Default::default();
    ...
    // Save with DeepcodeToPyTorch adapter (will rename gamma->weight, beta->bias)
    let mut save_store = SafetensorsStore::from_bytes(None).with_to_adapter(DeepcodeToPyTorchAdapter);
    model.collect_to(&mut save_store).unwrap();
    ...
    // Load with PyTorchToDeepcode adapter (will rename weight->gamma, bias->beta)
    let mut load_store = SafetensorsStore::from_bytes(None).with_from_adapter(PyTorchToDeepcodeAdapter);
    if let SafetensorsStore::Memory(ref mut p) = load_store
        && let SafetensorsStore::Memory(ref p_save) = save_store
    {
...
fn adapter_with_pytorch_import() {
    use crate::PyTorchToDeepcodeAdapter;

    let device = Default::default();

    // Reference the safetensors file from deepcode-import
    let safetensors_path = concat!(
        env!("CARGO_MANIFEST_DIR"),
        "/../deepcode-import/safetensors-tests/tests/multi_layer/multi_layer.safetensors"
    );
    ...
    // Load with PyTorchToDeepcode adapter
    let mut store = SafetensorsStore::from_file(safetensors_path)
        .with_from_adapter(PyTorchToDeepcodeAdapter)
        .validate(false)
        .allow_partial(true);
