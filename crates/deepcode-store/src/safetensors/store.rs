...
use alloc::vec;
use alloc::vec::Vec;
use deepcode_core::module::ParamId;
...
impl SafetensorsStore {
    /// Get the default metadata that includes Deepcode framework information.
    ///
    /// This includes:
    /// - `format`: "safetensors"
    /// - `producer`: "deepcode"
    /// - `version`: The version of deepcode-store crate (from CARGO_PKG_VERSION)
    ///
    ...
    pub fn default_metadata() -> HashMap<String, String> {
        ...
        metadata.insert("producer".to_string(), "deepcode".to_string());
    ...
    /// Clear all metadata including the default Deepcode framework metadata.
    ///
    ...
    /// Set the adapter for loading tensors (converting from source format to Deepcode).
    ...
    /// Set the adapter for saving tensors (converting from Deepcode to target format).
...
impl safetensors::View for TensorSnapshotAdapter {
    fn dtype(&self) -> safetensors::Dtype {
        // Convert from deepcode dtype to safetensors dtype
...
impl ModuleSnapshoter for SafetensorsStore {
    ...
    fn collect_from<B: Backend, M: ModuleSnapshot<B>>(
        ...
        // The to_adapter converts from Deepcode format to target format for saving
...
fn safetensors_to_snapshots_lazy(
    ...
    for (name, tensor_snapshot) in tensors.tensors() {
        ...
        let dtype = safetensor_dtype_to_deepcode(tensor_snapshot.dtype())?;
        ...
        let data_fn = alloc::rc::Rc::new(move || {
            ...
            let bytes = deepcode_tensor::Bytes::from_bytes_vec(tensor.data().to_vec());
            Ok(TensorData {
                bytes,
                shape: tensor.shape().to_vec(),
                dtype: safetensor_dtype_to_deepcode(tensor.dtype())
...
fn safetensors_to_snapshots_lazy_file(
    ...
    for (name, tensor_snapshot) in tensors.tensors() {
        ...
        let data_fn = alloc::rc::Rc::new(move || {
            ...
            Ok(TensorData {
                bytes: deepcode_tensor::Bytes::from_bytes_vec(tensor.data().to_vec()),
                shape: tensor.shape().to_vec(),
                dtype: safetensor_dtype_to_deepcode(tensor.dtype())
                    .map_err(|_| crate::TensorSnapshotError::DataError("Invalid dtype".into()))?,
            })
...
/// Helper to convert safetensors Dtype to deepcode DType.
fn safetensor_dtype_to_deepcode(dtype: safetensors::Dtype) -> Result<DType, SafetensorsStoreError> {
