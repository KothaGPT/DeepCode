...
//!
//! This module provides adapters that handle differences between PyTorch and Deepcode:
...
use deepcode_tensor::TensorData;
...
mod module_names {
    ...
    use deepcode_nn::{BatchNorm, GroupNorm, LayerNorm, Linear};
...
/// Adapter for converting from PyTorch format to Deepcode format
///
/// Handles:
/// - Linear layer weight transposition (PyTorch: [out, in] → Deepcode: [in, out])
/// - Normalization parameter renaming (weight → gamma, bias → beta)
#[derive(Debug, Clone, Default)]
pub struct PyTorchToDeepcodeAdapter;

impl ModuleAdapter for PyTorchToDeepcodeAdapter {
    fn adapt(&self, snapshot: &TensorSnapshot) -> TensorSnapshot {
        adapt_pytorch_tensor(snapshot, PyTorchConversionDirection::PyTorchToDeepcode)
    }
...
/// Adapter for converting from Deepcode format to PyTorch format
///
/// Handles:
/// - Linear layer weight transposition (Deepcode: [in, out] → PyTorch: [out, in])
/// - Normalization parameter renaming (gamma → weight, beta → bias)
#[derive(Debug, Clone, Default)]
pub struct DeepcodeToPyTorchAdapter;

impl ModuleAdapter for DeepcodeToPyTorchAdapter {
    fn adapt(&self, snapshot: &TensorSnapshot) -> TensorSnapshot {
        adapt_pytorch_tensor(snapshot, PyTorchConversionDirection::DeepcodeToPyTorch)
    }
    ...
    match container_type.as_str() {
        ...
        module_names::BATCH_NORM | module_names::LAYER_NORM | module_names::GROUP_NORM => {
            let new_name = match direction {
                PyTorchConversionDirection::PyTorchToDeepcode => match param_name {
                    "weight" => "gamma",
                    "bias" => "beta",
                    _ => return snapshot.clone(),
                },
                PyTorchConversionDirection::DeepcodeToPyTorch => match param_name {
                    "gamma" => "weight",
                    "beta" => "bias",
                    _ => return snapshot.clone(),
                },
...
mod tests {
    use super::*;
    use alloc::rc::Rc;
    use deepcode_tensor::{DType, TensorData};
    ...
    fn create_test_snapshot(path: &str, shape: Vec<usize>, container_type: &str) -> TensorSnapshot {
        ...
        TensorSnapshot::from_closure(
            ...
            deepcode_core::module::ParamId::new(),
        )
    }

    #[test]
    fn test_pytorch_to_deepcode_linear_weight() {
        let adapter = PyTorchToDeepcodeAdapter;
    ...
    #[test]
    fn test_pytorch_to_deepcode_norm_params() {
        let adapter = PyTorchToDeepcodeAdapter;
    ...
    #[test]
    fn test_deepcode_to_pytorch_linear_weight() {
        let adapter = DeepcodeToPyTorchAdapter;
    ...
    #[test]
    fn test_deepcode_to_pytorch_norm_params() {
        let adapter = DeepcodeToPyTorchAdapter;
