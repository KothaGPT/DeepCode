...
use deepcode_nn::conv::{Conv2d, Conv2dConfig};
use deepcode_nn::{Linear, LinearConfig};
use deepcode_tensor::Tensor;
use deepcode_tensor::backend::Backend;

/// Path to deepcode-import pytorch test files
fn pytorch_test_path(subdir: &str, filename: &str) -> PathBuf {
    PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .parent()
        .unwrap()
        .join("deepcode-import")
        .join("pytorch-tests")
        .join("tests")
        .join(subdir)
        .join(filename)
...
/// Path to deepcode-store test data files
...
mod adapter_tests {
    ...
    fn test_pytorch_adapter_always_applied() {
        // Test that PyTorchToDeepcodeAdapter is always applied internally
        let path = pytorch_test_path("linear", "linear.pt");
        ...
        // PyTorchToDeepcodeAdapter is always applied internally
        assert!(
            result.is_ok(),
            "Failed to load with internal PyTorchToDeepcodeAdapter: {:?}",
            result.err()
        );
    ...
    fn test_pytorch_adapter_with_filtering() {
        // Test that PyTorchToDeepcodeAdapter works with filtering
        let path = pytorch_test_path("linear", "linear.pt");
