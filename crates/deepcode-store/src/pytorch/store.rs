...
use crate::{
    ApplyResult, KeyRemapper, ModuleSnapshot, ModuleSnapshoter, PathFilter, PyTorchToDeepcodeAdapter,
    TensorSnapshot,
};
...
/// with automatic weight transformation using `PyTorchToDeepcodeAdapter`.
...
impl PytorchStore {
    ...
    /// # Example
    /// ```rust,ignore
    /// use deepcode_store::pytorch::PytorchStore;
    ///
...
impl ModuleSnapshoter for PytorchStore {
    ...
    fn apply_to<B: Backend, M: ModuleSnapshot<B>>(
        ...
        // Apply to module with PyTorchToDeepcodeAdapter (always used for PyTorch files)
        // This adapter handles:
        // - Transposing linear weights from PyTorch format to Deepcode format
        // - Renaming normalization parameters (gamma -> weight, beta -> bias)
        let result = module.apply(snapshots, None, Some(Box::new(PyTorchToDeepcodeAdapter)));
