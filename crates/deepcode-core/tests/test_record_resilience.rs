...
mod tests {
    ...
    use deepcode_ndarray::NdArrayDevice;
    ...
    fn test_tensor_serde() {
        let tensor: deepcode_tensor::Tensor<TestBackend, 1> =
            deepcode_tensor::Tensor::ones([1], &NdArrayDevice::default());
        let encoded = serde_json::to_string(&tensor).unwrap();
        let decoded: deepcode_tensor::Tensor<TestBackend, 1> = serde_json::from_str(&encoded).unwrap();
