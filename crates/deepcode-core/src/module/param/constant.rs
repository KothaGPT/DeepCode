...
macro_rules! constant {
    (module) => {
        type Record = deepcode::module::ConstantRecord;

        fn visit<V: deepcode::module::ModuleVisitor<B>>(&self, _visitor: &mut V) {
            // Nothing to do
        }

        fn map<M: deepcode::module::ModuleMapper<B>>(self, _mapper: &mut M) -> Self {
            self
        }
        ...
        fn into_record(self) -> Self::Record {
            deepcode::module::ConstantRecord::new()
        }
        ...
        fn collect_devices(&self, devices: deepcode::module::Devices<B>) -> deepcode::module::Devices<B> {
            devices
        }
    };
    ...
    ($type:ty) => {
        impl<B: deepcode::tensor::backend::Backend> deepcode::module::Module<B> for $type {
            constant!(module);
        }

        impl<B: deepcode::tensor::backend::AutodiffBackend> deepcode::module::AutodiffModule<B> for $type {
            constant!(ad_module, $type);
        }

        impl deepcode::module::ModuleDisplayDefault for $type {
            fn content(&self, content: deepcode::module::Content) -> Option<deepcode::module::Content> {
                let string = format!("{}", self);
                content.add_formatted(&string).optional()
            }
        }

        impl deepcode::module::ModuleDisplay for $type {}
    };
}
...
impl deepcode::module::ModuleDisplay for str {}
impl deepcode::module::ModuleDisplayDefault for str {
    fn content(&self, content: deepcode::module::Content) -> Option<deepcode::module::Content> {
...
mod tests {
    ...
    use deepcode_tensor::{Device, Tensor};
