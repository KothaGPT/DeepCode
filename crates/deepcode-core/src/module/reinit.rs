...
#[derive(Debug)]
/// Overrides float and int tensors of [deepcode modules](super::Module).
///
...
impl<B: Backend> ModuleMapper<B> for Reinitializer<B> {
    ...
    fn map_int<const D: usize>(
        &mut self,
        _id: ParamId,
        tensor: Tensor<B, D, deepcode_tensor::Int>,
    ) -> Tensor<B, D, deepcode_tensor::Int> {
        let device = tensor.device();
        let shape = tensor.shape();
        let num_elements = shape.num_elements();
    ...
    fn map_bool<const D: usize>(
        &mut self,
        _id: ParamId,
        tensor: Tensor<B, D, deepcode_tensor::Bool>,
    ) -> Tensor<B, D, deepcode_tensor::Bool> {
        tensor
    }
}
