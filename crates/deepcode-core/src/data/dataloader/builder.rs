...
mod tests {
    ...
    fn test_dataloader_slice_multi_device() {
        ...
        let (device1, device2) = (
            deepcode_ndarray::NdArrayDevice::Cpu,
            deepcode_ndarray::NdArrayDevice::Cpu,
        );

        #[cfg(all(test, feature = "test-tch"))]
        let (device1, device2) = (
            deepcode_tch::LibTorchDevice::Cuda(0),
            deepcode_tch::LibTorchDevice::Cuda(1),
        );

        #[cfg(all(test, feature = "test-wgpu"))]
        let (device1, device2) = (
            deepcode_wgpu::WgpuDevice::DiscreteGpu(0),
            deepcode_wgpu::WgpuDevice::DiscreteGpu(1),
        );

        #[cfg(all(test, feature = "test-cuda"))]
        let (device1, device2) = (deepcode_cuda::CudaDevice::new(0), deepcode_cuda::CudaDevice::new(1));
