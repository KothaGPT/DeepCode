    ...
    Send + Sync + core::default::Default + core::fmt::Debug + Clone
{
        ...
        R: Record<B>,
    {
        ...
        let item = DeepcodeRecord::new::<Self>(item);
    ...
    where
        R: Record<B>,
    {
        let item: DeepcodeRecord<R::Item<Self::Settings>, B> =
            self.load_item(&mut args).map_err(|err| {
                if let Ok(record) = self.load_item::<DeepcodeRecordNoItem>(&mut args) {
                    ...
                    if metadata.version != record.metadata.version {
                        message += format!(
                            "\nMetadata has a different Deepcode version: Actual {:?}, Expected {:?}",
...
fn recorder_metadata<R, B>() -> DeepcodeMetadata
where
    R: Recorder<B>,
    B: Backend,
{
    DeepcodeMetadata::new(
...
pub struct DeepcodeMetadata {
    ...
    /// Deepcode record version used to record the item.
    pub version: String,
...
pub struct DeepcodeRecord<I, B: Backend> {
    /// Metadata of the record.
    pub metadata: DeepcodeMetadata,
...
impl<I, B: Backend> DeepcodeRecord<I, B> {
...
pub struct DeepcodeRecordNoItem {
    /// Metadata of the record.
    pub metadata: DeepcodeMetadata,
}
...
mod tests {
    static FILE_PATH: &str = "/tmp/deepcode_test_record";

    use crate::TestBackend;

    use super::*;
    use deepcode_tensor::{Device, ElementConversion};
