...
/// Tensor representation with a reference to the [server handle](deepcl_runtime::server::Handle).
pub struct ArrayHandleRef<'a, R: Runtime> {
    pub handle: &'a deepcl_runtime::server::Handle,
...
impl<'a, R: Runtime> ArrayArg<'a, R> {
    ...
    pub unsafe fn from_raw_parts<E: CubePrimitive>(
        handle: &'a deepcl_runtime::server::Handle,
        length: usize,
        line_size: u8,
    ) -> Self {
        unsafe {
