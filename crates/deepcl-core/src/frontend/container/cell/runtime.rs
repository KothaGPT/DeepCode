...
use deepcl::frontend::assign::expand_no_check;
use deepcl::prelude::*;
use deepcl_ir::Operation;
...
pub struct RuntimeCellExpand<T: CubeType> {
    value: <T as deepcl::prelude::CubeType>::ExpandType,
}
...
impl<T: CubeType> deepcl::prelude::CubeType for RuntimeCell<T> {
    type ExpandType = RuntimeCellExpand<T>;
}
impl<T: CubeType> deepcl::prelude::IntoMut for RuntimeCellExpand<T> {
    fn into_mut(self, _scope: &mut deepcl::prelude::Scope) -> Self {
        Self {
...
impl<T: CubeType> deepcl::prelude::CubeDebug for RuntimeCellExpand<T> {}
