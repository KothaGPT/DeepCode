...
use crate::{self as deepcl, unexpanded};
use deepcl::prelude::*;
use deepcl_ir::{Branch, ElemType, ExpandElement, FloatKind, RangeLoop, Type, Variable};
use deepcl_macros::intrinsic;
    ...
    pub fn with_line_size(&self, #[comptime] line_size: u32) -> Slice<Line<E>, IO> {
        intrinsic!(|scope| {
            ...
            if current < line_size {
                ...
                let length = deepcl::frontend::div::expand(scope, self.length, ratio.into());
                let offset = deepcl::frontend::div::expand(scope, self.offset, ratio.into());
                out.length = length;
                out.offset = offset;
            } else {
                let ratio = current / line_size;
                let length = deepcl::frontend::mul::expand(scope, self.length, ratio.into());
                let offset = deepcl::frontend::mul::expand(scope, self.offset, ratio.into());
                out.length = length;
                out.offset = offset;
            }
...
impl<E: CubePrimitive, IO: SliceVisibility> Slice<E, IO> {
    ...
    pub fn __expand_new_expand(
        ...
        let length = deepcl::frontend::sub::expand(scope, end, start.clone());
...
impl<E: CubePrimitive, IO: SliceVisibility> IntoMut for SliceExpand<E, IO> {
    fn into_mut(self, _scope: &mut deepcl_ir::Scope) -> Self {
        self
    }
}
...
mod read_offset {
    ...
    pub fn expand<E: CubePrimitive>(
        scope: &mut deepcl::prelude::Scope,
        origin: SliceOriginExpand<E>,
        offset: <u32 as deepcl::prelude::CubeType>::ExpandType,
        index: <u32 as deepcl::prelude::CubeType>::ExpandType,
        line_size: Option<u32>,
        checked: bool,
    ) -> <E as deepcl::prelude::CubeType>::ExpandType {
        let index = deepcl::frontend::add::expand(scope, offset, index);
...
mod write_offset {
    ...
    pub fn expand<E: CubePrimitive>(
        ...
        offset: <u32 as deepcl::prelude::CubeType>::ExpandType,
        index: <u32 as deepcl::prelude::CubeType>::ExpandType,
        value: <E as deepcl::prelude::CubeType>::ExpandType,
        line_size: Option<u32>,
    ) {
        let index = deepcl::frontend::add::expand(scope, offset, index);
