use crate::prelude::*;
use crate::{self as deepcl, debug_print};
...
macro_rules! testgen_debug {
    () => {
        ...
        fn test_simple_call_debug() {
            ...
            deepcl_core::runtime_tests::debug::test_simple_call::<TestRuntime>(client);
        }

        #[test]
        fn test_nested_call_debug() {
            let client = TestRuntime::client(&Default::default());
            deepcl_core::runtime_tests::debug::test_nested_call::<TestRuntime>(client);
        }
        ...
        fn test_debug_print() {
            let client = TestRuntime::client(&Default::default());
            deepcl_core::runtime_tests::debug::test_debug_print::<TestRuntime>(client);
        }
    };
}
