...
macro_rules! testgen_launch {
    () => {
        ...
        fn test_launch_with_generics() {
            ...
            deepcl_core::runtime_tests::launch::test_kernel_with_generics::<TestRuntime, FloatType>(
                client,
            );
        }
        ...
        fn test_launch_without_generics() {
            let client = TestRuntime::client(&Default::default());
            deepcl_core::runtime_tests::launch::test_kernel_without_generics::<TestRuntime>(client);
        }

        #[test]
        fn test_launch_with_comptime_tag() {
            let client = TestRuntime::client(&Default::default());
            deepcl_core::runtime_tests::launch::test_kernel_with_comptime_tag::<TestRuntime>(
                client,
            );
        }
        ...
        fn test_launch_with_max_shared() {
            let client = TestRuntime::client(&Default::default());
            deepcl_core::runtime_tests::launch::test_kernel_max_shared::<TestRuntime>(client);
        }
    };
}
