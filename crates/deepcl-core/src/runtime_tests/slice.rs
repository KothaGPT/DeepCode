use crate::{self as deepcl, as_bytes, as_type};
use deepcl::prelude::*;
...
macro_rules! testgen_slice {
    () => {
        ...
        fn test_slice_select() {
            ...
            deepcl_core::runtime_tests::slice::test_slice_select::<TestRuntime, FloatType>(client);
        }

        #[test]
        fn test_slice_len() {
            let client = TestRuntime::client(&Default::default());
            deepcl_core::runtime_tests::slice::test_slice_len::<TestRuntime, FloatType>(client);
        }

        #[test]
        fn test_slice_for() {
            let client = TestRuntime::client(&Default::default());
            deepcl_core::runtime_tests::slice::test_slice_for::<TestRuntime, FloatType>(client);
        }

        #[test]
        fn test_slice_mut_assign() {
            let client = TestRuntime::client(&Default::default());
            deepcl_core::runtime_tests::slice::test_slice_mut_assign::<TestRuntime, FloatType>(
                client,
            );
        }
        ...
        fn test_slice_mut_len() {
            let client = TestRuntime::client(&Default::default());
            deepcl_core::runtime_tests::slice::test_slice_mut_len::<TestRuntime>(client);
        }
    };
}
