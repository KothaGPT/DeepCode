use crate::{self as deepcl, as_bytes, prelude::barrier::BarrierLevel};
use barrier::Barrier;
use deepcl::prelude::*;
use deepcl_ir::SemanticType;
...
macro_rules! testgen_barrier {
    () => {
        ...
        fn test_barrier_async_copy() {
            ...
            deepcl_core::runtime_tests::barrier::test_async_copy::<TestRuntime, FloatType>(client);
        }

        #[test]
        fn test_barrier_memcpy_async_one_load() {
            let client = TestRuntime::client(&Default::default());
            deepcl_core::runtime_tests::barrier::test_memcpy_one_load::<TestRuntime, FloatType>(
                client,
            );
        }
        ...
        fn test_barrier_memcpy_async_two_loads() {
            let client = TestRuntime::client(&Default::default());
            deepcl_core::runtime_tests::barrier::test_memcpy_two_loads::<TestRuntime, FloatType>(
                false, client,
            );
        }
