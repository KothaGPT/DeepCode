...
use alloc::vec;
use alloc::vec::Vec;
use deepcl_common::{ExecutionMode, bytes::Bytes, profile::ProfileDuration};
    ...
    Channel: ComputeChannel<Server>,
{
    ...
    /// # Safety
    ///
    /// This is highly unsafe and should probably only be used by the DeepCL/Burn projects for now.
    ...
    pub fn read(&self, handles: Vec<Handle>) -> Vec<Bytes> {
        deepcl_common::reader::read_sync(self.read_async(handles))
    }
    ...
    pub fn read_one(&self, handle: Handle) -> Bytes {
        deepcl_common::reader::read_sync(self.read_async(vec![handle])).remove(0)
    }
    ...
    pub fn read_tensor(&self, descriptors: Vec<CopyDescriptor<'_>>) -> Vec<Bytes> {
        deepcl_common::reader::read_sync(self.read_tensor_async(descriptors))
    }
    ...
    fn change_client_sync(
        ...
        let data = deepcl_common::future::block_on(read).unwrap();
