...
/// Represents the global configuration for DeepCL, combining profiling, autotuning, and compilation settings.
#[derive(Default, Clone, Debug, serde::Serialize, serde::Deserialize)]
pub struct GlobalConfig {
    /// Configuration for profiling DeepCL operations.
    #[serde(default)]
...
impl GlobalConfig {
    ...
    ///
    /// If no configuration is set, it attempts to load one from `deepcl.toml` or `DeepCL.toml` in the
    ...
    pub fn override_from_env(mut self) -> Self {
        ...
        if let Ok(val) = std::env::var("DEEPCL_DEBUG_LOG") {
        ...
        if let Ok(val) = std::env::var("DEEPCL_DEBUG_OPTION") {
            match val.as_str() {
                "debug" => {
                    self.compilation.logger.level = CompilationLogLevel::Full;
                    self.profiling.logger.level = ProfilingLogLevel::Medium;
        ...
        if let Ok(val) = std::env::var("DEEPCL_AUTOTUNE_LEVEL") {
            match val.as_str() {
                "minimal" | "0" => {
    ...
    // Loads configuration from `deepcl.toml` or `DeepCL.toml` in the current directory or its parents.
    //
    ...
    fn from_current_dir() -> Self {
        ...
        loop {
            if let Ok(content) = Self::from_file_path(dir.join("deepcl.toml")) {
                return content;
            }
...
/// How to format deepcl type names.
...
mod test {
    ...
    #[test]
    fn test_format_name() {
        let full_name = "burn_deepcl::kernel::unary_numeric::unary_numeric::UnaryNumeric<f32, burn_deepcl::tensor::base::CubeTensor<_>::copy::Copy, deepcl_cuda::runtime::CudaRuntime>";
