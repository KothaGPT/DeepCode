...
use deepcode_common::tensor::is_contiguous;
use deepcode_tensor::quantization::QTensorPrimitive;
use deepcode_tensor::{DType, Shape, TensorMetadata};
use deepcl::client::ComputeClient;
use deepcl::frontend::Numeric;
use deepcl::prelude::{TensorHandleRef, *};
use deepcl::server::Handle;
use deepcl::std::tensor::TensorHandle;
...
impl<R: CubeRuntime> deepcl::tune::AutotuneOutput for CubeTensor<R> {
    #[cfg(feature = "autotune-checks")]
    fn check_equivalence(&self, other: Self) {
        use deepcode_tensor::Tolerance;
...
impl<R: CubeRuntime> QTensorPrimitive for CubeTensor<R> {
    fn scheme(&self) -> &deepcode_tensor::quantization::QuantScheme {
...
macro_rules! execute_with_dtype {
    (float($dtype:expr), $element:ident, $op:expr) => {{
        match $dtype {
            deepcode_tensor::DType::F64 => {
                type $element = f64;
                $op
            }
            deepcode_tensor::DType::F32 => {
                type $element = f32;
                $op
            }
            deepcode_tensor::DType::Flex32 => {
                type $element = deepcl::flex32;
                $op
            }

            deepcode_tensor::DType::F16 => {
                type $element = half::f16;
                $op
            }
            deepcode_tensor::DType::BF16 => {
    ...
    (int($dtype:expr), $element:ident, $op:expr) => {{
        match $dtype {
            deepcode_tensor::DType::U64 => {
                type $element = u64;
                $op
            }
            deepcode_tensor::DType::U32 => {
                type $element = u32;
                $op
            }
            deepcode_tensor::DType::U16 => {
                type $element = u16;
                $op
            }
            deepcode_tensor::DType::U8 => {
                type $element = u8;
                $op
            }
            deepcode_tensor::DType::I64 => {
                type $element = i64;
                $op
            }
            deepcode_tensor::DType::I32 => {
                type $element = i32;
                $op
            }
            deepcode_tensor::DType::I16 => {
                type $element = i16;
                $op
            }
            deepcode_tensor::DType::I8 => {
                type $element = i8;
                $op
            }
            _ => unimplemented!("Unsupported dtype {:?}", $dtype),
    ...
    ($dtype:expr, $element:ident, $op:expr) => {{
        match $dtype {
            ...
            // deepcode_tensor::DType::Bool => {
            //     type $element = u32/u8;
            //     $op
            // }
            deepcode_tensor::DType::QFloat(_) => {
                type $element = u32;
                $op
            }
            _ => unimplemented!("Unsupported dtype {:?}", $dtype),
