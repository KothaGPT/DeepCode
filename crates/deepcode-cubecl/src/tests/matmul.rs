#[deepcode_tensor_testgen::testgen(matmul)]
mod tests {
    use super::*;
    use deepcode_deepcl::kernel::matmul::{MatmulStrategy, matmul};
    use deepcode_tensor::{Shape, Tensor, TensorPrimitive};
    ...
    mod padding {
        use super::*;
        use deepcode_deepcl::kernel::matmul::padding::{crop, pad_round};
        ...
        fn crop_same_shape_should_be_unchanged_shape() {
            ...
            let tensor =
                TestTensor::random([row, col], deepcode_tensor::Distribution::Default, &device);
            let expected_shape = [row, col].into();
        ...
        S: Into<Shape<D>>,
    {
        ...
        let x = ReferenceTensor::random(
            shape_lhs,
            deepcode_tensor::Distribution::Uniform(-1.0, 1.0),
            &ref_tensor_device,
        );
