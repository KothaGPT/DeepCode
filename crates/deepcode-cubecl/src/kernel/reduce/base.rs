...
pub use deepcl::reduce::instructions::{ArgMax, ArgMin, Mean, Prod, Sum};
use deepcl::{
    AutotuneKey,
...
pub struct SumAutotuneKey {
    ...
    pub dtype: deepcode_tensor::DType,
...
) -> Result<CubeTensor<Run>, deepcl::reduce::ReduceError> {
...
pub fn reduce_dim<Run: CubeRuntime, In: CubeElement, Out: CubeElement, Acc: CubeElement>(
    ...
    let output = init_reduce_output::<Run, In, Out>(&input, dim).ok_or(
        deepcl::reduce::ReduceError::InvalidAxis {
            axis: dim,
    ...
    let result = match strategy {
        ReduceStrategy::Unspecified => deepcl::reduce::reduce::<Run, (In, Acc), Out, ReduceFn>(
            &client,
            input.as_handle_ref(),
            output.as_handle_ref(),
            dim,
        ...
        ReduceStrategy::Specific(strategy) => {
            deepcl::reduce::reduce::<Run, (In, Acc), Out, ReduceFn>(
                &client,
                input.as_handle_ref(),
                output.as_handle_ref(),
                dim,
...
pub enum ReduceStrategy {
    ...
    Specific(deepcl::reduce::ReduceStrategy),
