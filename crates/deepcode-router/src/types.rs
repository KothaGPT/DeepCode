...
use deepcode_ir::{BackendIr, OperationIr, TensorHandle, TensorIr};
use deepcode_tensor::{
...
macro_rules! impl_multi_backend_types {
    ...
    ($module_name:ident, $DefaultBackend:ident, $($OtherBackend:ident),+) => {
        ...
        /// - `ByteBridge`: a simple multi-backend bridge that transfers tensors via the underlying [tensor data](deepcode_tensor::TensorData).
        ///
        ...
        pub mod $module_name {
            ...
            impl<$DefaultBackend: Backend, $($OtherBackend: Backend),+> deepcode_common::device::Device for MultiDevice<$DefaultBackend, $($OtherBackend),+> {
            ...
            impl<$DefaultBackend: BackendIr, $($OtherBackend: BackendIr),+> RunnerClient for MultiRunnerClient<$DefaultBackend, $($OtherBackend),+>
            {
               type Device = MultiDevice<$DefaultBackend, $($OtherBackend),+>;
                ...
                fn register_float_tensor(&self, shape: Shape, dtype: deepcode_tensor::FloatDType) -> RouterTensor<Self> {
                    match self {
                        Self::$DefaultBackend(runner) => {
...
#[cfg(test)]
mod tests {
    use deepcode_tensor::{Tensor, backend::Backend};
