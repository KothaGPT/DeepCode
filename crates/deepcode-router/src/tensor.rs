...
use deepcode_ir::{TensorId, TensorIr, TensorStatus};
use deepcode_tensor::{DType, Shape, TensorData, TensorMetadata};
    ...
    fn drop(&mut self) {
        ...
        match self.status(count) {
            TensorStatus::ReadWrite => {
                ...
                self.client.register(deepcode_ir::OperationIr::Drop(ir));
            }
            TensorStatus::ReadOnly => {}
