use deepcl_common::device::{Device, DeviceId};
...
pub enum WgpuDevice {
    ...
    /// the `DEEPCL_WGPU_DEFAULT_DEVICE` environment variable. This variable is spelled as if i was a WgpuDevice,
    /// so for example DEEPCL_WGPU_DEFAULT_DEVICE=IntegratedGpu(1) or DEEPCL_WGPU_DEFAULT_DEVICE=Cpu
    #[default]
    DefaultDevice,
    ...
    /// Use an externally created, existing, wgpu setup. This is helpful when using DeepCL in conjunction
    /// with some existing wgpu setup (eg. egui or bevy), as resources can be transferred in & out of DeepCL.
    ///
    /// # Notes
    ///
