...
use deepcl_common::{future, profile::TimingMethod};

use deepcl_core::{CubeCount, CubeDim, Runtime, ir::TargetProperties};
pub use deepcl_runtime::memory_management::MemoryConfiguration;
use deepcl_runtime::memory_management::MemoryDeviceProperties;
use deepcl_runtime::{
    ComputeRuntime, channel,
    client::ComputeClient,
    logging::{ProfileLevel, ServerLogger},
};
use deepcl_runtime::{DeviceProperties, memory_management::HardwareProperties};
...
impl Default for RuntimeOptions {
    fn default() -> Self {
        ...
        let tasks_max = match std::env::var("DEEPCL_WGPU_MAX_TASKS") {
            Ok(value) => value
                .parse::<usize>()
                .expect("DEEPCL_WGPU_MAX_TASKS should be a positive integer."),
...
/// Useful when you want to share a device between DeepCL and other wgpu-dependent libraries.
///
/// # Note
///
...
pub(crate) fn create_client_on_setup(
    ...
    // However, some algorithms do rely on having this information eg. deepcl-reduce uses max subgroup size _even_ when
    ...
    if features.contains(wgpu::Features::SHADER_FLOAT32_ATOMIC) {
        use deepcl_core::ir::{ElemType, FloatKind, StorageType};
        use deepcl_runtime::TypeUsage;
...
fn get_device_override() -> Option<WgpuDevice> {
    ...
    std::env::var("DEEPCL_WGPU_DEFAULT_DEVICE")
        .ok()
        .and_then(|var| {
            ...
            if override_device.is_none() {
                log::warn!("Unknown DEEPCL_WGPU_DEVICE override {var}");
            }
            override_device
        })
}
