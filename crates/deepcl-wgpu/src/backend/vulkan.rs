...
use deepcl_runtime::{DeviceProperties, EnumSet, MmaConfig, Plane, TypeUsage};
use deepcl_spirv::{GLCompute, SpirvCompiler, SpirvKernel};
...
pub fn register_vulkan_features(
    ...
    props: &mut deepcl_runtime::DeviceProperties,
...
fn register_types(props: &mut DeviceProperties, ext_feat: &ExtendedFeatures<'_>) {
    use deepcl_core::ir::{ElemType, FloatKind, IntKind, StorageType};
...
fn dump_spirv(
    ...
    if let Ok(dir) = std::env::var("DEEPCL_DEBUG_SPIRV") {
