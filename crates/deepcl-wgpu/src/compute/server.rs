...
use deepcl_common::profile::{ProfileDuration, TimingMethod};
use deepcl_common::stream_id::StreamId;
use deepcl_core::future::DynFut;
use deepcl_core::server::{ProfileError, ProfilingToken, ServerCommunication};
use deepcl_core::{
...
use deepcl_runtime::memory_management::{MemoryAllocationMode, offset_handles};
use deepcl_runtime::stream::scheduler::{
...
impl ComputeServer for WgpuServer {
    ...
    fn memory_usage(
        &mut self,
        stream_id: StreamId,
    ) -> deepcl_runtime::memory_management::MemoryUsage {
        self.scheduler.execute_streams(vec![stream_id]);
        let stream = self.scheduler.stream(&stream_id);
