...
use deepcl_common::profile::{Duration, Instant, ProfileDuration, ProfileTicks};
use deepcl_core::server::{ProfileError, ProfilingToken};
...
fn create_resolve_buffer(device: &wgpu::Device, count: u32) -> wgpu::Buffer {
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("DeepCL gpu -> cpu resolve buffer"),
...
fn create_map_buffer(device: &wgpu::Device, count: u32) -> wgpu::Buffer {
    device.create_buffer(&wgpu::BufferDescriptor {
        label: Some("DeepCL gpu -> cpu map buffer"),
...
fn get_cur_timestamp(queue: &wgpu::Queue, device: &wgpu::Device) -> u64 {
    ...
    let query_set = device.create_query_set(&wgpu::QuerySetDescriptor {
        label: Some("DeepCL gpu -> cpu sync query_set"),
    ...
    fn new_query_set(
        ...
        let query_set = if let Some(pool) = self.query_set_pool.pop() {
            ...
            device.create_query_set(&QuerySetDescriptor {
                label: Some("DeepCL profile queries"),
