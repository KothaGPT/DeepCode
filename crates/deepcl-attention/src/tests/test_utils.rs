        ...
        $(
            impl Sampleable for $t
            {
                fn sample<R: Runtime>(client: &ComputeClient<R::Server, R::Channel>, shape: &[usize], seed: u64) -> TensorHandle::<R, Self> {
                    deepcl_random::seed(seed);
                    let output = TensorHandle::<R, Self>::empty(client, shape.to_vec());

                    deepcl_random::random_uniform::<R, Self>(&client, Self::from_int(-50), Self::from_int(50), output.as_ref());
...
impl Sampleable for flex32 {
    fn sample<R: Runtime>(
        ...
        deepcl_random::random_uniform::<R, f32>(
            client,
            f32::from_int(-1),
            f32::from_int(1),
            output.as_ref(),
...
impl Sampleable for bool {
    fn sample<R: Runtime>(
        ...
        deepcl_random::random_bernoulli::<R, f32>(client, 0.5, output.as_ref());
