...
impl<E: TchElement> IntTensorOps<Self> for LibTorch<E> {
    fn int_from_data(data: TensorData, device: &LibTorchDevice) -> TchTensor {
        match data.dtype {
            deepcode_tensor::DType::I64 => TchTensor::from_data::<i64>(data, (*device).into()),
    ...
    fn int_slice(tensor: TchTensor, slices: &[deepcode_tensor::Slice]) -> TchTensor {
    ...
    fn bitwise_and_scalar(
        lhs: IntTensor<Self>,
        rhs: deepcode_tensor::ops::IntElem<Self>,
    ) -> IntTensor<Self> {
