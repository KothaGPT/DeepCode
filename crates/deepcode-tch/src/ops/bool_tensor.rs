...
use deepcode_tensor::ops::IntTensor;
use deepcode_tensor::{Shape, TensorData, TensorMetadata, backend::Backend, ops::BoolTensorOps};

impl<E: TchElement> BoolTensorOps<Self> for LibTorch<E> {
    fn bool_from_data(data: TensorData, device: &LibTorchDevice) -> TchTensor {
        match data.dtype {
            deepcode_tensor::DType::Bool => TchTensor::from_data::<bool>(data, (*device).into()),
    ...
    fn bool_slice(tensor: TchTensor, slices: &[deepcode_tensor::Slice]) -> TchTensor {
