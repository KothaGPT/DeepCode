...
Deepcode is a deep learning framework in the Rust programming language. Therefore, it goes without
...
thing to do to start using Deepcode.

## Creating a Deepcode application
...
```console
cargo new my_deepcode_app
```

This will initialize the `my_deepcode_app` project directory with a `Cargo.toml` file and a `src`
...
Then, add Deepcode as a dependency:

```console
cargo add deepcode --features wgpu
```
...
That's it, you're ready to start! You have a project configured with Deepcode and the WGPU backend,
...
To bring any of the Deepcode module or item into scope, a `use` declaration is added.
...
```rust, ignore
use deepcode::{tensor::Tensor, backend::Wgpu};
```
...
Deepcode comes with a variety of things in its core library. When creating a new model or using an
