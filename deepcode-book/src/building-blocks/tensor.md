...
Deepcode Tensors are defined by the number of dimensions D in its declaration as opposed to its shape.
...
Deepcode Tensors are primarily initialized using the `from_data()` method which takes the `TensorData`
...
Almost all Deepcode operations take ownership of the input tensors. Therefore, reusing a tensor multiple
...
available for further operations. Deepcode Tensors like most complex primitives do not implement the
...
operations into a single kernel ([deepcode-fusion](https://deepcode.dev/docs/deepcode_fusion/index.htmls)). For
...
them useful only for data preprocessing or inference-only model implementations. With Deepcode, you can
...
[full documentation](https://docs.rs/deepcode/latest/deepcode/tensor/struct.Tensor.html).

### Basic Operations

Those operations are available for all tensor kinds: `Int`, `Float`, and `Bool`.

| Deepcode                                         | PyTorch Equivalent                                                        |
| -------------------------------------------- | ------------------------------------------------------------------------- |
...
Deepcode provides flexible options for displaying tensor information, allowing you to control the level
...
For more fine-grained control over tensor printing, Deepcode provides a `PrintOptions` struct and a
`set_print_options` function:

```rust
use deepcode::tensor::{set_print_options, PrintOptions};
...
- `edge_items`: Number of items to show at the beginning and end of each dimension when summarizing
  ...
  Deepcode provides a utility function `check_closeness` to compare two tensors and assess their
  ...
  ```rust
  use deepcode::tensor::{check_closeness, Tensor};
  type B = deepcode::backend::NdArray;
  ...
  also an essential tool when verifying the accuracy of imported models, ensuring that the Deepcode
