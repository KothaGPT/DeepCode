# Custom `deepcl` Kernel

In this section, you will learn how to create your own custom operation by writing your own kernel
with the deepcl compiler frontend. We will take the example of a common workflow in the deep
learning field, where we create a kernel to fuse multiple operations together. Note that `deepcode` does
...
[examples directory](https://github.com/kothagpt/deepcode/tree/main/examples/custom-deepcl-kernel).
...
```rust, ignore
/// We create our own Backend trait that extends the Deepcode backend trait.
pub trait Backend: deepcode::tensor::backend::Backend {
...
/// We create our own AutodiffBackend trait that extends the Deepcode autodiff backend trait.
pub trait AutodiffBackend: Backend + deepcode::tensor::backend::AutodiffBackend {}
```

In our project, we can use these traits instead of the
`deepcode::tensor::backend::{Backend, AutodiffBackend}` traits provided by Deepcode. Deepcode's user APIs
...
Now, let's proceed to write the fused kernel using the `deepcl` compiler frontend. To keep things
...
[`deepcl` Book](https://github.com/tracel-ai/deepcl/tree/f5b63076a01a5c03ea9ed20799d3eeaf776b45da/deepcl-book).
...
automatically implements the trait for `deepcode-cuda`, `deepcode-wgpu` as well as fusion.
...
For the backward pass, we will leverage the backend implementation from `deepcode-autodiff`, which is
actually generic over the backend. Instead of crafting our own `deepcl` kernel for the backward
    ...
    fn fused_matmul_add_relu(
        ...
        impl<B: Backend> Backward<B, 3> for FusedMatmulAddReluBackward {
            ...
            fn backward(
                ...
                // function in the basic Deepcode backend trait.
...
In this guide, we've implemented a fused kernel using the `deepcl` compiler frontend, enabling
execution on any GPU and any `deepcl` backend. By delving into the inner workings of both the JIT
...
As we conclude this guide, we hope that you have gained insights into Deepcode's world of backend
