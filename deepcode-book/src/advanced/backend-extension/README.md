...
Deepcode aims to be the most flexible deep learning framework. While it's crucial to maintain
compatibility with a wide variety of backends, Deepcode provides the ability to extend the functionality
...
As you can observe, most types in Deepcode are generic over the Backend trait. This might give the
impression that Deepcode operates at a high level over the backend layer. However, making the trait
...
pub trait Backend: deepcode::tensor::backend::Backend {
    fn my_new_function(tensor: B::TensorPrimitive<2>) -> B::TensorPrimitive<2> {
        // You can define a basic implementation reusing the Deepcode Backend API.
...
```rust, ignore
impl<E: TchElement> Backend for deepcode_tch::LibTorch<E> {
   fn my_new_function(tensor: TchTensor<E, 2>) -> TchTensor<E, 2> {
      // My Tch implementation
   }
}

impl<E: NdArrayElement> Backend for deepcode_ndarray::NdArray<E> {
...
```rust, ignore
impl<B: Backend> Backend for deepcode_autodiff::Autodiff<B> {
...
impl<E: TchElement> Backend for deepcode_autodiff::Autodiff<deepcode_tch::LibTorch<E>> {
   fn my_new_function(tensor: AutodiffTensor<E, 2>) -> AutodiffTensor<E, 2> {
...
`deepcl` compiler frontend is the recommended method of implementing custom kernels, since it
supports multiple backends, including `wgpu` and `CUDA`, and is the way first-party `deepcode` kernels
are written.
