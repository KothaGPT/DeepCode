...
where we create a kernel to fuse multiple operations together. Note that `deepcode` does this
...
[examples directory](https://github.com/kothagpt/deepcode/tree/main/examples/custom-wgpu-kernel).
...
you will have to activate the `template` feature of Deepcode in your `cargo.toml`.
...
impl<E: FloatElement> KernelSource for FusedMatmulAddRelu<E> {
    ...
    fn id(&self) -> deepcl::KernelId {
        deepcl::KernelId::new::<Self>().info(self.cube_dim)
    }
}
```
