...
The first step is to create a project and add the different Deepcode dependencies. Start by creating a
...
As [mentioned previously](../getting-started.md#creating-a-deepcode-application), this will initialize
your `guide` project directory with a `Cargo.toml` and a `src/main.rs` file.

In the `Cargo.toml` file, add the `deepcode` dependency with `train`, `vision` and `wgpu` features.
...
deepcode = { version = "~0.19", features = ["std", "tui", "train", "vision", "wgpu", "fusion"], default-features = false }
# deepcode = { version = "~0.19", features = ["train", "vision", "wgpu"] }
```
...
1. You can create a deep learning module with the `#[derive(Module)]` attribute on top of a struct.
   ...
   formatter. For ease of use, the `Module` trait implementation is automatically handled by Deepcode so
...
2. Note that the struct is generic over the [`Backend`](../building-blocks/backend.md) trait. The
   ...
   In Deepcode, the `Backend` trait enables you to run tensor operations using different implementations
...
network modules already built with Deepcode use the `forward` nomenclature, simply because it is
