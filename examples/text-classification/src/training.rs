...
#[cfg(feature = "ddp")]
use deepcode::collective::{AllReduceStrategy, CollectiveConfig};
#[cfg(not(feature = "ddp"))]
use deepcode::train::LearningStrategy;
use deepcode::{
...
pub fn train<B: AutodiffBackend, D: TextClassificationDataset + 'static>(
    ...
    let learner = LearnerBuilder::new(artifact_dir)
        ...
        .metric_train_numeric(LearningRateMetric::new())
        .with_file_checkpointer(CompactRecorder::new())
        .learning_strategy(deepcode::train::ddp(devices, collective_config))
        .num_epochs(config.num_epochs)
        .summary()
        .build(model, optim, lr_scheduler);
