...
[![Live Demo](https://img.shields.io/badge/live-demo-brightgreen)](https://deepcode.dev/demo)
...
The inference components of `deepcode` with the `ndarray` backend can be built with `#![no_std]`. This
...
include deepcode dependencies without `std`).
...
compiler optimizes and includes only used `deepcode` routines. 1,509,747 bytes out of Wasm's 1,866,491
byte file is the model's parameters. The rest of 356,744 bytes contain all the code (including
`deepcode`'s `nn` components, the data deserialization library, and math operations).
