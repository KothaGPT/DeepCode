use deepcode::{backend::Autodiff, tensor::backend::Backend};
use simple_regression::{inference, training};

static ARTIFACT_DIR: &str = "/tmp/deepcode-example-regression";
...
mod ndarray {
    use deepcode::backend::ndarray::{NdArray, NdArrayDevice};
...
#[cfg(feature = "tch-gpu")]
mod tch_gpu {
    use deepcode::backend::libtorch::{LibTorch, LibTorchDevice};
...
#[cfg(feature = "wgpu")]
mod wgpu {
    use deepcode::backend::wgpu::{Wgpu, WgpuDevice};
...
#[cfg(feature = "remote")]
mod remote {
    use deepcode::backend::{RemoteBackend, remote::RemoteDevice};
