...
mod device_sharing_wgpu {
    use deepcl::wgpu::{AutoGraphicsApi, GraphicsApi, WgpuDevice, WgpuSetup};
    use wgpu::Features;

    pub fn create_wgpu_setup_from_raw() -> WgpuSetup {
        deepcl::future::block_on(create_wgpu_setup_from_raw_async())
    }
    ...
    pub fn assert_wgpu_device_existing(device: &WgpuDevice) {
        assert!(
            matches!(device, deepcl::wgpu::WgpuDevice::Existing(_)),
